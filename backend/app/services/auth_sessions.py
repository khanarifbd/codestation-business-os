...
_DEVICE_ID_HEADER = "x-business-os-device-id"
_DEVICE_ID_MIN_LENGTH = 16
_DEVICE_ID_MAX_LENGTH = 200
...
def _request_device_id_hash(request: Request) -> str | None:
    """Return a non-secret hash used only to group sessions by browser profile.

    The browser device id is an opaque random cookie generated by the trusted
    web BFF. It is never used as an authentication credential. API clients that
    do not send it keep the existing one-session-per-login behavior.
    """

    raw = (request.headers.get(_DEVICE_ID_HEADER) or "").strip()
    if not (_DEVICE_ID_MIN_LENGTH <= len(raw) <= _DEVICE_ID_MAX_LENGTH):
        return None
...
def create_user_session(
    ...
    *,
    ...
    device_id_hash = _request_device_id_hash(request)

    # Serialize explicit session creation for this account. This prevents two
    # near-simultaneous sign-ins from the same browser from both surviving as
    # active sessions after they replace the same prior device session.
    ...
    if device_id_hash:
        db.execute(
            ...
            .where(
                ...
                UserSession.token_version == int(user.auth_token_version or 0),
            )
            .values(
                revoked_at=now,
                revoked_reason="new_sign_in_same_device",
            )
        )
