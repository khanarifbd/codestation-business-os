...
import {
  ...
  LogOut, Menu, Receipt, ReceiptText, Scale, TrendingUp, UserRound, Users, UsersRound, WalletCards, X,
  type LucideIcon,
} from "lucide-react";
...
const employeeNavigation: NavigationItem[] = [
  { label: "My Work", icon: BriefcaseBusiness, href: "/dashboard/my-work", permissions: ["projects.work"] },
  { label: "Projects", icon: FolderKanban, href: "/dashboard/my-projects", permissions: ["projects.work"] },
...
function MobileBottomNavigation({ pathname, items, drawerOpen, onOpenDrawer }: { pathname: string; items: NavigationItem[]; drawerOpen: boolean; onOpenDrawer: () => void }) {
  const hasPrimaryActive = items.some((item) => isActive(pathname, item.href));
  const moreActive = drawerOpen || !hasPrimaryActive;
  return <nav aria-label="Mobile primary navigation" className="fixed inset-x-0 bottom-0 z-40 border-t border-neutral-200 bg-white/95 backdrop-blur lg:hidden">
    <div className="grid min-h-16 items-stretch px-1" style={{ gridTemplateColumns: `repeat(${items.length + 1}, minmax(0, 1fr))` }}>
      {items.map(({ label, icon: Icon, href }) => { const active=isActive(pathname,href); return <Link key={`${label}-${href}`} href={href} aria-current={active?"page":undefined} className={`flex min-w-0 flex-col items-center justify-center gap-1 px-1 py-2 text-[10px] font-medium transition ${active?"text-neutral-950":"text-neutral-400 hover:text-neutral-700"}`}><span className={`flex size-8 items-center justify-center rounded-xl transition ${active?"bg-neutral-950 text-white":"bg-transparent"}`}><Icon className="size-[18px]"/></span><span className="max-w-full truncate">{label}</span></Link>;})}
      <button type="button" aria-label="Open app drawer" aria-expanded={drawerOpen} onClick={onOpenDrawer} className={`flex min-w-0 flex-col items-center justify-center gap-1 px-1 py-2 text-[10px] font-medium transition ${moreActive?"text-neutral-950":"text-neutral-400 hover:text-neutral-700"}`}><span className={`flex size-8 items-center justify-center rounded-xl transition ${moreActive?"bg-neutral-950 text-white":"bg-transparent"}`}><Menu className="size-[18px]"/></span><span>More</span></button>
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const employeeLanding=hasAnyPermission(permissions,["projects.work"])?"/dashboard/my-work":"/dashboard/hr/me";
  useEffect(()=>{if(clientOnly&&pathname!=="/dashboard/client-portal"&&!pathname.startsWith("/dashboard/profile")){router.replace("/dashboard/client-portal");return;}if(employeeSelfService&&pathname==="/dashboard"){router.replace(employeeLanding);return;}if(employeeSelfService&&pathname.startsWith("/dashboard/projects")){router.replace(pathname.replace("/dashboard/projects","/dashboard/my-projects"));}},[clientOnly,employeeLanding,employeeSelfService,pathname,router]);
  const navigation=useMemo(()=>{if(clientOnly)return[clientPortalItem];if(employeeSelfService)return employeeNavigation;const items=[...staffNavigation];if(hasClientRelationship)items.splice(1,0,clientPortalItem);return items;},[clientOnly,employeeSelfService,hasClientRelationship]);
  const mobilePrimaryNavigation=useMemo<NavigationItem[]>(()=>{if(clientOnly)return[{label:"Portal",icon:Building2,href:"/dashboard/client-portal"},{label:"Profile",icon:UserRound,href:"/dashboard/profile"}];if(employeeSelfService)return employeeNavigation.filter(item=>hasAnyPermission(permissions,item.permissions)).slice(0,3);const items:NavigationItem[]=[];const add=(item:NavigationItem|null)=>{if(!item||!hasAnyPermission(permissions,item.permissions)||items.some((existing)=>existing.href===item.href))return;items.push(item);};add({label:"Home",icon:LayoutDashboard,href:"/dashboard",permissions:["dashboard.view"]});if(hasAnyPermission(permissions,["projects.work"]))add({label:"Work",icon:BriefcaseBusiness,href:"/dashboard/my-work",permissions:["projects.work"]});else add({label:"Projects",icon:FolderKanban,href:"/dashboard/projects",permissions:["projects.view"]});if(hasAnyPermission(permissions,["clients.view"]))add({label:"Clients",icon:Users,href:"/dashboard/clients",permissions:["clients.view"]});else if(hasAnyPermission(permissions,["crm.view"]))add({label:"CRM",icon:ClipboardList,href:"/dashboard/crm",permissions:["crm.view"]});else add({label:"Orders",icon:ReceiptText,href:"/dashboard/orders",permissions:["orders.view"]});if(hasAnyPermission(permissions,["finance.view","capital.view"]))add({label:"Finance",icon:WalletCards,href:"/dashboard/accounting",permissions:["finance.view","capital.view"]});else add({label:"Reports",icon:BarChart3,href:"/dashboard/reports",permissions:["reports.view"]});return items.slice(0,4);},[clientOnly,employeeSelfService,permissions]);
  ...
  return <div className="min-h-screen bg-neutral-100 text-neutral-950 lg:flex">
    <aside className="hidden h-screen w-64 shrink-0 border-r border-neutral-200 bg-white p-4 lg:sticky lg:top-0 lg:flex lg:flex-col"><div className="flex items-center gap-3 px-3 py-4"><div className="flex size-10 items-center justify-center rounded-xl border border-neutral-200 bg-white p-2 shadow-sm"><BrandMark className="h-full w-full object-contain" /></div><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-neutral-400">CodeStation AI</p><h1 className="mt-0.5 text-lg font-semibold">{employeeSelfService?"Employee Portal":"Business OS"}</h1></div></div><div className="px-1 pb-4"><WorkspaceSwitcher onContextChange={setWorkspaceContext}/></div><div className="mt-1 flex-1 overflow-y-auto pb-4">{employeeSelfService?<p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-neutral-400">My workspace</p>:null}<Navigation pathname={pathname} items={navigation} permissions={permissions}/></div><div className="space-y-2 border-t pt-3"><ProfileLink profile={profile} active={profileActive}/><button type="button" onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-neutral-500 hover:bg-neutral-100 hover:text-neutral-950"><LogOut className="size-4"/>Sign out</button></div></aside>
    <div className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-white/95 px-4 backdrop-blur lg:hidden"><div className="flex min-w-0 items-center gap-2.5"><div className="flex size-9 shrink-0 items-center justify-center rounded-xl border border-neutral-200 bg-white p-2"><BrandMark className="h-full w-full object-contain" /></div><div className="min-w-0"><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-neutral-400">{employeeSelfService?"Employee Portal":"CodeStation AI"}</p><p className="max-w-[190px] truncate text-sm font-semibold">{workspaceContext?.organization.name??"Business OS"}</p></div></div><button type="button" aria-label="Open app drawer" aria-expanded={mobileOpen} onClick={()=>setMobileOpen(true)} className="flex size-10 items-center justify-center rounded-xl border"><Menu className="size-5"/></button></div>
    {mobileOpen?<div className="fixed inset-0 z-50 lg:hidden"><button type="button" aria-label="Close app drawer" onClick={()=>setMobileOpen(false)} className="absolute inset-0 bg-black/35"/><aside className="absolute inset-y-0 left-0 flex w-[88vw] max-w-96 flex-col bg-white p-4 shadow-2xl"><div className="flex items-center justify-between px-3 py-3"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl border border-neutral-200 bg-white p-2"><BrandMark className="h-full w-full object-contain" /></div><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-neutral-400">CodeStation AI</p><h2 className="mt-0.5 text-lg font-semibold">{employeeSelfService?"Employee Portal":"Business OS"}</h2></div></div><button type="button" aria-label="Close app drawer" onClick={()=>setMobileOpen(false)} className="flex size-9 items-center justify-center rounded-lg border"><X className="size-4"/></button></div><div className="px-1 pb-4"><WorkspaceSwitcher onContextChange={setWorkspaceContext}/></div><div className="mt-2 flex-1 overflow-y-auto pb-4"><p className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.16em] text-neutral-400">{employeeSelfService?"My workspace":"All apps"}</p><Navigation pathname={pathname} items={navigation} permissions={permissions} onNavigate={()=>setMobileOpen(false)}/></div><div className="space-y-2 border-t pt-3"><ProfileLink profile={profile} active={profileActive} onNavigate={()=>setMobileOpen(false)}/><button type="button" onClick={()=>void logout()} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-neutral-600 hover:bg-neutral-100"><LogOut className="size-4"/>Sign out</button></div></aside></div>:null}
    <div className={`min-w-0 flex-1 pb-[calc(4rem+env(safe-area-inset-bottom))] lg:pb-0 ${pathname==="/dashboard"?"[&>main>div>aside]:!hidden [&>main>div]:!max-w-none":""}`}>{children}</div>
    <MobileBottomNavigation pathname={pathname} items={mobilePrimaryNavigation} drawerOpen={mobileOpen} onOpenDrawer={()=>setMobileOpen(true)}/>
  </div>;
}