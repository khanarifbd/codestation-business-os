...
function isActive(pathname: string, href: string) {
  ...
  if (href === "/dashboard/finance") {
    return pathname === href || (
      ...
      !pathname.startsWith("/dashboard/finance/transfers") &&
      !pathname.startsWith("/dashboard/finance/controls") &&
      !pathname.startsWith("/dashboard/finance/auto-post")
    );
  }
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-neutral-100 text-neutral-950 lg:flex">
    <aside className="hidden h-screen w-60 shrink-0 border-r border-neutral-200 bg-white p-4 lg:sticky lg:top-0 lg:flex lg:flex-col">
      ...
      <div className="mt-3 flex-1 overflow-y-auto pb-4"><Navigation pathname={pathname} /></div>
    ...
    <div className="sticky top-0 z-40 flex h-16 items-center justify-between border-b bg-white/95 px-4 backdrop-blur lg:hidden">
      <div><p className="text-[10px] font-medium uppercase tracking-[0.18em] text-neutral-400">CodeStation AI</p><p className="text-sm font-semibold">Business OS</p></div>
      <button type="button" aria-label="Open navigation" aria-expanded={mobileOpen} onClick={() => setMobileOpen(true)} className="flex size-10 items-center justify-center rounded-xl border"><Menu className="size-5" /></button>
    </div>

    {mobileOpen ? <div className="fixed inset-0 z-50 lg:hidden">
      <button type="button" aria-label="Close navigation" onClick={() => setMobileOpen(false)} className="absolute inset-0 bg-black/35" />
      <aside className="absolute inset-y-0 left-0 flex w-[86vw] max-w-80 flex-col bg-white p-4 shadow-2xl">
        <div className="flex items-center justify-between px-3 py-3"><div><p className="text-xs font-medium uppercase tracking-[0.18em] text-neutral-400">CodeStation AI</p><h2 className="mt-1 text-lg font-semibold">Business OS</h2></div><button type="button" onClick={() => setMobileOpen(false)} className="flex size-9 items-center justify-center rounded-lg border"><X className="size-4" /></button></div>
        <div className="mt-2 flex-1 overflow-y-auto pb-4"><Navigation pathname={pathname} onNavigate={() => setMobileOpen(false)} /></div>
        <button type="button" onClick={() => void logout()} className="mt-3 flex items-center gap-3 rounded-xl border px-3 py-2.5 text-sm text-neutral-600"><LogOut className="size-4" />Sign out</button>
      </aside>
    </div> : null}
