...
const sourceCards = [
  { value: "invoice" as SourceType, title: "Invoice payment", help: "Customer paid an existing invoice", icon: FileText },
...
export function MoneyInWorkspace() {
  ...
  const sourceInvoice = useMemo(() => sourceType === "invoice"
    ...
    : sourceType === "project"
      ...
      : sourceType === "order"
        ...
        : null, [sourceType, form.source_id, invoices]);
  ...
  const sourceCurrency = sourceInvoice?.currency ?? selectedProject?.currency ?? selectedOrder?.currency ?? selectedClient?.currency ?? accounts.find((account) => account.id === form.account_id)?.currency ?? "";
  ...
  function reset(type: SourceType) {
    setSourceType(type);
    setForm({ source_id: "", account_id: "", amount: "", date: today(), method: "bank_transfer", category_id: "", description: "", reference: "", notes: "" });
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setForm({ source_id: "", account_id: "", amount: "", date: today(), method: "bank_transfer", category_id: "", description: "", reference: "", notes: "" });
      await load();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not record money received"); }
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-2xl border bg-white p-5"><h2 className="text-lg font-semibold">Where did this money come from?</h2><p className="mt-1 text-sm text-neutral-500">Choose the real business event. Business OS handles the accounting in the background.</p>
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="flex justify-end md:col-span-2 lg:col-span-3"><button disabled={saving || loading} className="inline-flex items-center gap-2 rounded-xl bg-neutral-950 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50"><ArrowDownLeft className="size-4" />{saving ? "Saving…" : "Confirm money received"}</button></div>
      </form>
    </section>
