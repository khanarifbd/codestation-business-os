...
type TaskEditValues = { title: string; description: string; milestone_id: string; priority: string; assignee_employee_id: string; planned_start_date: string; due_date: string; estimated_hours: string };
...
function isOverdueDate(dueDate: string | null, status: string) { return Boolean(dueDate && dueDate < localDateKey() && !["completed", "cancelled"].includes(status)); }
...
export default function ProjectWorkspacePage() {
  ...
  const [modal, setModal] = useState<"milestone" | "milestone_edit" | "task" | "task_edit" | "progress" | "document" | "credential" | "credential_edit" | "team" | null>(null);
  ...
  async function updateMilestone(values: { title: string; description: string; due_date: string }) {
    if (!selectedMilestone) return;
    setSaving(true); setError(null);
    try {
      await api(`/${projectId}/milestones/${selectedMilestone.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: values.title, description: values.description || null, due_date: values.due_date || null }) });
      setModal(null); setSelectedMilestone(null); setMessage("Milestone schedule updated."); await refresh();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to update milestone."); }
    finally { setSaving(false); }
  }

  async function createTask(values: TaskEditValues) {
    setSaving(true); setError(null);
    try {
  ...
  async function updateTask(values: TaskEditValues) {
    if (!selectedTask) return;
    setSaving(true); setError(null);
    try {
      await api(`/${projectId}/tasks/${selectedTask.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        title: values.title, description: values.description || null, milestone_id: values.milestone_id || null, priority: values.priority,
        assignee_employee_id: values.assignee_employee_id || null, planned_start_date: values.planned_start_date || null, due_date: values.due_date || null,
        estimated_minutes: values.estimated_hours ? Math.round(Number(values.estimated_hours) * 60) : null,
      }) });
      const taskCode = selectedTask.task_code;
      setModal(null); setSelectedTask(null); setMessage(`Schedule updated for ${taskCode}.`); await refresh();
  ...
  return <main className="min-h-screen bg-neutral-100 p-5 sm:p-7 lg:p-9"><div className="mx-auto max-w-[1500px]">
    ...
    <section className="mt-5">
      ...
      {tab === "milestones" && project.access.allowed_tabs.includes("milestones") ? <Milestones rows={workspace.milestones} onAdd={canManageExecution ? () => setModal("milestone") : undefined} onEdit={canManageExecution ? (item) => { setSelectedMilestone(item); setModal("milestone_edit"); } : undefined} /> : null}
      {tab === "tasks" && project.access.allowed_tabs.includes("tasks") ? <Tasks rows={workspace.tasks} currentEmployeeId={project.access.current_employee_id} canManage={canManageExecution} onAdd={canManageExecution ? () => setModal("task") : undefined} onEdit={canManageExecution ? (task) => { setSelectedTask(task); setModal("task_edit"); } : undefined} onProgress={(task) => { setSelectedTask(task); setModal("progress"); }} /> : null}
  ...
  {modal === "milestone_edit" && selectedMilestone && canManageExecution ? <EditMilestoneModal saving={saving} milestone={selectedMilestone} tasks={workspace.tasks} projectDueDate={project.due_date} onClose={() => { setModal(null); setSelectedMilestone(null); }} onSave={updateMilestone} /> : null}
  ...
  {modal === "task_edit" && selectedTask && canManageExecution ? <EditTaskModal saving={saving} task={selectedTask} milestones={workspace.milestones} members={project.members} onClose={() => { setModal(null); setSelectedTask(null); }} onSave={updateTask} /> : null}
...
function Milestones({ rows, onAdd, onEdit }: { rows: MilestoneRow[]; onAdd?: () => void; onEdit?: (item: MilestoneRow) => void }) { return <Card title="Milestones" action={onAdd ? <AddButton label="Add milestone" onClick={onAdd} /> : undefined}><div className="space-y-3">{rows.length ? rows.map((item) => { const overdue = isOverdueDate(item.due_date, item.status); return <div key={item.id} className={`rounded-xl border p-4 ${overdue ? "border-red-200 bg-red-50/30" : ""}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold">{item.title}</p><p className={`mt-1 text-xs ${overdue ? "font-medium text-red-600" : "text-neutral-400"}`}>Due {item.due_date || "not set"}{overdue ? " · Overdue" : ""}</p></div><div className="flex items-center gap-2"><Badge value={item.status} />{onEdit ? <button onClick={() => onEdit(item)} className="inline-flex h-8 items-center gap-1.5 rounded-lg border bg-white px-2.5 text-xs font-semibold"><Pencil className="size-3.5" />Edit</button> : null}</div></div><div className="mt-4"><div className="mb-1 text-right text-xs font-medium">{item.progress_percent}%</div><Progress value={item.progress_percent} /></div>{item.description ? <p className="mt-3 text-sm text-neutral-600">{item.description}</p> : null}</div>; }) : <Empty text="No milestones yet." />}</div></Card>; }

function Tasks({ rows, currentEmployeeId, canManage, onAdd, onEdit, onProgress }: { rows: TaskRow[]; currentEmployeeId: string | null; canManage: boolean; onAdd?: () => void; onEdit?: (task: TaskRow) => void; onProgress: (task: TaskRow) => void }) { return <Card title="Tasks" action={onAdd ? <AddButton label="Add task" onClick={onAdd} /> : undefined}><div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-sm"><thead className="text-xs uppercase text-neutral-400"><tr><th className="pb-3">Task</th><th className="pb-3">Milestone</th><th className="pb-3">Assignee</th><th className="pb-3">Status</th><th className="pb-3">Due</th><th className="pb-3">Progress</th><th className="pb-3 text-right">Action</th></tr></thead><tbody className="divide-y">{rows.map((task) => { const canUpdate = canManage || Boolean(currentEmployeeId && task.assignee_employee_id === currentEmployeeId); const overdue = isOverdueDate(task.due_date, task.status); return <tr key={task.id} className={overdue ? "bg-red-50/30" : ""}><td className="py-4 pr-4"><p className="font-medium">{task.task_code} · {task.title}</p><p className="mt-1 text-xs capitalize text-neutral-400">{task.priority} priority</p></td><td className="py-4 pr-4">{task.milestone_title || "—"}</td><td className="py-4 pr-4">{task.assignee_name || "Unassigned"}</td><td className="py-4 pr-4"><Badge value={task.status} /></td><td className="py-4 pr-4"><span className={overdue ? "font-medium text-red-600" : ""}>{task.due_date || "—"}</span>{overdue ? <span className="mt-1 block text-[11px] font-semibold uppercase tracking-wide text-red-500">Overdue</span> : null}</td><td className="py-4 pr-4"><div className="w-32"><Progress value={task.progress_percent} /></div><span className="mt-1 block text-xs text-neutral-400">{task.progress_percent}%</span></td><td className="py-4 text-right"><div className="flex justify-end gap-2">{canManage && onEdit ? <button onClick={() => onEdit(task)} className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-semibold"><Pencil className="size-3.5" />Edit</button> : null}{canUpdate ? <button disabled={task.status === "cancelled"} onClick={() => onProgress(task)} className="rounded-lg border px-3 py-2 text-xs font-semibold disabled:opacity-40">Update Work</button> : <span className="text-xs text-neutral-400">View only</span>}</div></td></tr>; })}</tbody></table>{!rows.length ? <Empty text="No tasks yet." /> : null}</div></Card>; }
...
function EditMilestoneModal({ saving, milestone, tasks, projectDueDate, onClose, onSave }: { saving:boolean; milestone:MilestoneRow; tasks:TaskRow[]; projectDueDate:string|null; onClose:()=>void; onSave:(v:{title:string;description:string;due_date:string})=>Promise<void> }) { const [title,setTitle]=useState(milestone.title); const [description,setDescription]=useState(milestone.description || ""); const [due,setDue]=useState(milestone.due_date || ""); const activeTaskDates=tasks.filter((task)=>task.milestone_id===milestone.id && task.due_date && !["completed","cancelled"].includes(task.status)).map((task)=>task.due_date as string).sort(); const latestTaskDue=activeTaskDates.at(-1) || null; const changed=due !== (milestone.due_date || ""); const taskConflict=Boolean(due && latestTaskDue && latestTaskDue > due); const projectConflict=Boolean(due && projectDueDate && due > projectDueDate); return <Modal title="Edit milestone schedule" onClose={onClose}><div className="rounded-xl border bg-neutral-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold">{milestone.title}</p><p className="mt-1 text-xs text-neutral-500">Current due date: {milestone.due_date || "Not set"}</p></div><Badge value={milestone.status} /></div><div className="mt-3"><Progress value={milestone.progress_percent} /></div></div>{changed ? <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">Schedule change: <span className="font-semibold">{milestone.due_date || "Not set"}</span> → <span className="font-semibold">{due || "Not set"}</span>. Saving will refresh project schedule and overdue indicators automatically.</div> : null}{taskConflict ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Some active tasks under this milestone are due as late as <span className="font-semibold">{latestTaskDue}</span>. Consider rescheduling those tasks too so the plan stays consistent.</div> : null}{projectConflict ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This milestone would extend beyond the project due date <span className="font-semibold">{projectDueDate}</span>. Review the overall project delivery date as well.</div> : null}<Field label="Milestone title"><input value={title} maxLength={220} onChange={(e)=>setTitle(e.target.value)} className="control" /></Field><Field label="Delivery / due date"><input type="date" value={due} onChange={(e)=>setDue(e.target.value)} className="control" /></Field><Field label="Description"><textarea value={description} onChange={(e)=>setDescription(e.target.value)} className="textarea" /></Field><ModalActions saving={saving} disabled={!title.trim()} onClose={onClose} onSave={() => void onSave({title:title.trim(),description,due_date:due})} /></Modal>; }
function TaskModal({ saving, milestones, members, onClose, onSave }: { saving:boolean; milestones:MilestoneRow[]; members:ProjectMember[]; onClose:()=>void; onSave:(v:TaskEditValues)=>Promise<void> }) { const [title,setTitle]=useState(""); const [description,setDescription]=useState(""); const [milestone,setMilestone]=useState(""); const [priority,setPriority]=useState("normal"); const [assignee,setAssignee]=useState(""); const [start,setStart]=useState(""); const [due,setDue]=useState(""); const [hours,setHours]=useState(""); return <Modal title="Add task" onClose={onClose}><div className="grid gap-4 sm:grid-cols-2"><Field label="Task title"><input value={title} onChange={(e)=>setTitle(e.target.value)} className="control" /></Field><Field label="Milestone"><select value={milestone} onChange={(e)=>setMilestone(e.target.value)} className="control"><option value="">No milestone</option>{milestones.map((m)=><option key={m.id} value={m.id}>{m.title}</option>)}</select></Field><Field label="Assignee"><select value={assignee} onChange={(e)=>setAssignee(e.target.value)} className="control"><option value="">Unassigned</option>{members.map((m)=><option key={m.employee_id} value={m.employee_id}>{m.full_name}</option>)}</select></Field><Field label="Priority"><select value={priority} onChange={(e)=>setPriority(e.target.value)} className="control"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></Field><Field label="Planned start"><input type="date" value={start} onChange={(e)=>setStart(e.target.value)} className="control" /></Field><Field label="Due date"><input type="date" value={due} onChange={(e)=>setDue(e.target.value)} className="control" /></Field><Field label="Estimated hours"><input type="number" min="0" step="0.25" value={hours} onChange={(e)=>setHours(e.target.value)} className="control" /></Field><label className="sm:col-span-2 text-sm font-medium">Description<textarea value={description} onChange={(e)=>setDescription(e.target.value)} className="textarea" /></label></div><ModalActions saving={saving} disabled={!title.trim()} onClose={onClose} onSave={() => void onSave({title:title.trim(),description,milestone_id:milestone,priority,assignee_employee_id:assignee,planned_start_date:start,due_date:due,estimated_hours:hours})} /></Modal>; }
function EditTaskModal({ saving, task, milestones, members, onClose, onSave }: { saving:boolean; task:TaskRow; milestones:MilestoneRow[]; members:ProjectMember[]; onClose:()=>void; onSave:(v:TaskEditValues)=>Promise<void> }) { const [title,setTitle]=useState(task.title); const [description,setDescription]=useState(task.description || ""); const [milestone,setMilestone]=useState(task.milestone_id || ""); const [priority,setPriority]=useState(task.priority); const [assignee,setAssignee]=useState(task.assignee_employee_id || ""); const [start,setStart]=useState(task.planned_start_date || ""); const [due,setDue]=useState(task.due_date || ""); const [hours,setHours]=useState(task.estimated_minutes === null ? "" : String(task.estimated_minutes / 60)); const selectedMilestone=milestones.find((item)=>item.id===milestone) || null; const invalidRange=Boolean(start && due && due < start); const milestoneConflict=Boolean(due && selectedMilestone?.due_date && due > selectedMilestone.due_date); const dueChanged=due !== (task.due_date || ""); const wasOverdue=isOverdueDate(task.due_date, task.status); return <Modal title={`Edit task · ${task.task_code}`} onClose={onClose}><div className="rounded-xl border bg-neutral-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-semibold">{task.title}</p><p className="mt-1 text-xs text-neutral-500">{task.assignee_name || "Unassigned"} · {task.progress_percent}% complete</p></div><Badge value={task.status} /></div></div>{dueChanged ? <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-700">Due date: <span className="font-semibold">{task.due_date || "Not set"}</span> → <span className="font-semibold">{due || "Not set"}</span>. {wasOverdue ? "If the new date is current or future, the Overdue count will clear automatically after save." : "Project overdue indicators will refresh automatically after save."}</div> : null}{invalidRange ? <div className="mt-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">Due date cannot be before the planned start date.</div> : null}{milestoneConflict ? <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This task would be due after its milestone delivery date <span className="font-semibold">{selectedMilestone?.due_date}</span>. Consider extending the milestone too.</div> : null}<div className="grid gap-4 sm:grid-cols-2"><Field label="Task title"><input value={title} maxLength={220} onChange={(e)=>setTitle(e.target.value)} className="control" /></Field><Field label="Milestone"><select value={milestone} onChange={(e)=>setMilestone(e.target.value)} className="control"><option value="">No milestone</option>{milestones.map((m)=><option key={m.id} value={m.id}>{m.title}</option>)}</select></Field><Field label="Assignee"><select value={assignee} onChange={(e)=>setAssignee(e.target.value)} className="control"><option value="">Unassigned</option>{members.map((m)=><option key={m.employee_id} value={m.employee_id}>{m.full_name}</option>)}</select></Field><Field label="Priority"><select value={priority} onChange={(e)=>setPriority(e.target.value)} className="control"><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="urgent">Urgent</option></select></Field><Field label="Planned start"><input type="date" value={start} onChange={(e)=>setStart(e.target.value)} className="control" /></Field><Field label={wasOverdue ? "Reschedule due date" : "Due date"}><input type="date" value={due} onChange={(e)=>setDue(e.target.value)} className="control" /></Field><Field label="Estimated hours"><input type="number" min="0" step="0.25" value={hours} onChange={(e)=>setHours(e.target.value)} className="control" /></Field><label className="sm:col-span-2 text-sm font-medium">Description<textarea value={description} onChange={(e)=>setDescription(e.target.value)} className="textarea" /></label></div><div className="mt-4 rounded-xl border bg-neutral-50 px-4 py-3 text-xs leading-5 text-neutral-500">Schedule details are separate from execution history. Use <span className="font-semibold text-neutral-700">Update Work</span> for progress/status/time entries; those work logs remain historical records.</div><ModalActions saving={saving} disabled={!title.trim() || invalidRange} onClose={onClose} onSave={() => void onSave({title:title.trim(),description,milestone_id:milestone,priority,assignee_employee_id:assignee,planned_start_date:start,due_date:due,estimated_hours:hours})} /></Modal>; }
