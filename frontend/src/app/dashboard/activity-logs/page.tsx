...
export default function TenantActivityLogsPage() {
  ...
  async function load(cursor?: string, append = false) {
    ...
    const response = await fetch(`/api/tenant/activity-logs?${query.toString()}`, {
      cache: "no-store",
    });
      ...
      <div className="mx-auto max-w-[1400px]">
        <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight">Company Activity Logs</h1>
            <p className="mt-2 text-sm text-neutral-500">
              Admin-only audit history for the currently selected company workspace.
            </p>
          </div>
