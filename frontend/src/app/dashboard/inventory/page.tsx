...
export default function InventoryPage() {
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1500px] space-y-6">
    ...
    {modal ? <ModalShell title={modalTitle(modal, editingId)} onClose={() => { setModal(null); setEditingId(null); }}>
      {modal === "item" ? <form onSubmit={submitItem} className="grid gap-4 md:grid-cols-2"><Select label="Item type" value={itemForm.item_type} onChange={(value) => setItemForm({ ...itemForm, item_type: value, track_inventory: value === "stock_item" })} options={[["stock_item", "Stock product"], ["non_stock_item", "Non-stock product"], ["service", "Service"]]} /><Input label="SKU / Code" value={itemForm.sku} onChange={(value) => setItemForm({ ...itemForm, sku: value })} /><Input label="Name" value={itemForm.name} onChange={(value) => setItemForm({ ...itemForm, name: value })} /><SearchableSelect label="Category" value={itemForm.category_id} onValueChange={(value) => setItemForm({ ...itemForm, category_id: value })} options={activeCategoryOptions} placeholder="Uncategorized" searchPlaceholder="Search categories..." /><Input label="Barcode" optional value={itemForm.barcode} onChange={(value) => setItemForm({ ...itemForm, barcode: value })} /><Input label="Unit" value={itemForm.unit} onChange={(value) => setItemForm({ ...itemForm, unit: value })} /><CurrencyInput label="Currency" value={itemForm.currency} onChange={(value) => setItemForm({ ...itemForm, currency: value })} /><Input label="Selling price" type="number" value={itemForm.selling_price} onChange={(value) => setItemForm({ ...itemForm, selling_price: value })} />{itemForm.item_type === "stock_item" ? <><Input label="Opening / standard cost" type="number" value={itemForm.standard_cost} onChange={(value) => setItemForm({ ...itemForm, standard_cost: value })} /><Input label="Low-stock reorder level" type="number" value={itemForm.reorder_level} onChange={(value) => setItemForm({ ...itemForm, reorder_level: value })} /></> : null}<Select label="Sales tax" required={false} value={itemForm.tax_code_id} onChange={(value) => setItemForm({ ...itemForm, tax_code_id: value })} options={[["", "No tax"], ...salesTaxes.map((tax) => [tax.id, `${tax.code} · ${tax.rate}%`])]} /><Input label="Description" optional value={itemForm.description} onChange={(value) => setItemForm({ ...itemForm, description: value })} />{editingId ? <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={itemForm.is_active} onChange={(event) => setItemForm({ ...itemForm, is_active: event.target.checked })} />Active item</label> : null}<Save saving={saving} text={editingId ? "Update item" : "Add item"} /></form> : null}
...
function CurrencyInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <SearchableSelect label={label} value={value} onValueChange={onChange} options={CURRENCY_OPTIONS} placeholder="Select currency" searchPlaceholder="Search currency or code..." clearable={false} />; }
function Select({ label, value, onChange, options, required = true }: { label: string; value: string; onChange: (value: string) => void; options: string[][]; required?: boolean }) { return <label className="grid gap-1.5 text-sm"><span className="font-medium">{label}</span><select required={required} value={value} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl border bg-white px-3">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></label>; }
