...
const emptyLine = (): LineInput => ({ item_name: "", description: "", quantity: "1", unit_price: "0", discount_percent: "0", tax_rate: "0", unit: "service", item_type: "service" });
...
export default function OrderCommercialPage() {
  ...
  async function createBilling() {
    const result = await post("/billing-milestones", { title: billingTitle, due_date: billingDue || null, items: payloadLines(billingLines) }, "billing-create");
    if (result) { setBillingOpen(false); setBillingTitle(""); setBillingDue(""); setBillingLines([emptyLine()]); }
  }
  ...
  const metrics = [
    ...
    ["Scheduled", summary.scheduled_value], ["Billed", summary.billed_value], ["Paid", summary.paid_value],
    ["Accounts receivable", summary.accounts_receivable], ["Remaining to bill", summary.remaining_to_bill],
  ];
  ...
  return <main className="min-h-screen bg-neutral-100 p-5 sm:p-8 lg:p-10"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{metrics.map(([label, value]) => <div key={label} className="rounded-2xl border bg-white p-5"><p className="text-xs font-medium uppercase tracking-[0.1em] text-neutral-400">{label}</p><p className="mt-2 text-xl font-semibold">{money(value, summary.currency)}</p></div>)}</div>
    {Number(summary.remaining_to_bill) > 0 && summary.staged_billing_enabled ? <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><strong>{money(summary.remaining_to_bill, summary.currency)}</strong> of approved contract value is still unbilled. Create another billing milestone/invoice or approve a scope reduction before treating the commercial work as fully closed.</div> : null}
