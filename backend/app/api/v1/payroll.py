...
def _profile_read(profile: SalaryProfile, employee_code: str, employee_name: str) -> SalaryProfileRead:
    return SalaryProfileRead(
        ...
        default_allowances=profile.default_allowances,
        default_deductions=profile.default_deductions,
...
def _period_read(period: PayrollPeriod) -> PayrollPeriodRead:
...
def _entry_reads(db: DbSession, organization_id: str, run_id: str) -> list[PayrollEntryRead]:
    entries = db.scalars(
        select(PayrollEntry)
        .where(PayrollEntry.organization_id == organization_id, PayrollEntry.run_id == run_id)
    ...
    return [
        PayrollEntryRead(
            ...
            employee_code=names.get(item.employee_id, ("—", "—"))[0],
            employee_name=names.get(item.employee_id, ("—", "—"))[1],
...
def _run_read(db: DbSession, organization_id: str, run: PayrollRun, include_entries: bool = True) -> PayrollRunRead:
    ...
    return PayrollRunRead(
        ...
        allowance_total=run.allowance_total,
        deduction_total=run.deduction_total,
        tax_total=run.tax_total,
        net_total=run.net_total,
        paid_account_id=run.paid_account_id,
...
def _recalculate_run(db: DbSession, run: PayrollRun) -> None:
    row = db.execute(
        select(
            ...
            func.coalesce(func.sum(PayrollEntry.allowance_total), 0),
            func.coalesce(func.sum(PayrollEntry.deduction_total), 0),
            func.coalesce(func.sum(PayrollEntry.tax_amount), 0),
            func.coalesce(func.sum(PayrollEntry.net_pay), 0),
        ).where(
            PayrollEntry.organization_id == run.organization_id,
            PayrollEntry.run_id == run.id,
        )
    ).one()
...
def payroll_meta(db: DbSession, tenant: PayrollViewer) -> PayrollMeta:
    employees = [
        PayrollEmployeeOption(id=row.id, employee_code=row.employee_code, full_name=row.full_name)
        for row in _employee_rows(db, tenant.organization_id)
    ]
    accounts = db.scalars(
    ...
    return PayrollMeta(
        employees=employees,
        accounts=[PayrollAccountOption(id=a.id, name=a.name, currency=a.currency, is_active=a.is_active) for a in accounts],
...
def list_salary_profiles(db: DbSession, tenant: PayrollViewer):
    profiles = db.scalars(
        ...
        .where(SalaryProfile.organization_id == tenant.organization_id)
        .order_by(SalaryProfile.is_active.desc(), SalaryProfile.created_at.desc())
    ).all()
...
def create_salary_profile(payload: SalaryProfileCreate, request: Request, db: DbSession, tenant: PayrollManager):
    ...
    if payload.pay_frequency not in {"monthly", "biweekly", "weekly"}:
        raise HTTPException(status_code=400, detail="Unsupported pay frequency")

    existing = db.scalars(
        select(SalaryProfile)
        .where(
            SalaryProfile.organization_id == tenant.organization_id,
    ...
    profile = SalaryProfile(
        ...
        base_salary=_money(payload.base_salary),
        default_allowances=_component_dicts(payload.default_allowances),
        default_deductions=_component_dicts(payload.default_deductions),
    ...
    record_activity(
        db,
        action="payroll.salary_profile.created",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="salary_profile",
        entity_id=profile.id,
        after={"employee_id": profile.employee_id, "currency": profile.currency, "base_salary": str(profile.base_salary)},
        request=request,
    )
    db.commit()
    db.refresh(profile)
...
def update_salary_profile(profile_id: str, payload: SalaryProfileUpdate, request: Request, db: DbSession, tenant: PayrollManager):
    profile = db.scalar(
        select(SalaryProfile)
        .where(SalaryProfile.id == profile_id, SalaryProfile.organization_id == tenant.organization_id)
        .with_for_update()
    )
    if profile is None:
    ...
    if changes.get("base_salary") is not None:
        changes["base_salary"] = _money(changes["base_salary"])
    if changes.get("default_allowances") is not None:
        changes["default_allowances"] = _component_dicts(payload.default_allowances or [])
    if changes.get("default_deductions") is not None:
        changes["default_deductions"] = _component_dicts(payload.default_deductions or [])
    for key, value in changes.items():
        setattr(profile, key, value)
    db.flush()
    record_activity(
        db,
        action="payroll.salary_profile.updated",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="salary_profile",
        entity_id=profile.id,
        before=before,
        after={"base_salary": str(profile.base_salary), "is_active": profile.is_active},
        request=request,
    )
    db.commit()
    db.refresh(profile)
...
def list_periods(db: DbSession, tenant: PayrollViewer):
    periods = db.scalars(
        ...
        .where(PayrollPeriod.organization_id == tenant.organization_id)
        .order_by(PayrollPeriod.period_start.desc())
        .limit(100)
    ).all()
    return [_period_read(item) for item in periods]
...
def create_period(payload: PayrollPeriodCreate, request: Request, db: DbSession, tenant: PayrollManager):
    ...
    record_activity(
        ...
        entity_id=item.id,
        after={"name": item.name, "period_start": str(item.period_start), "period_end": str(item.period_end)},
        request=request,
    )
    db.commit()
    db.refresh(item)
    return _period_read(item)
...
def list_runs(db: DbSession, tenant: PayrollViewer):
    runs = db.scalars(
        select(PayrollRun)
        .where(PayrollRun.organization_id == tenant.organization_id)
...
def create_run(payload: PayrollRunCreate, request: Request, db: DbSession, tenant: PayrollManager):
    period = db.scalar(
        select(PayrollPeriod)
        .where(PayrollPeriod.id == payload.period_id, PayrollPeriod.organization_id == tenant.organization_id)
        .with_for_update()
    )
    if period is None:
        raise HTTPException(status_code=404, detail="Payroll period not found")
    if period.status != "open":
        raise HTTPException(status_code=409, detail="Payroll period is not open")
    ...
    profiles = db.scalars(
        select(SalaryProfile).where(
            SalaryProfile.organization_id == tenant.organization_id,
            SalaryProfile.currency == currency,
            ...
            or_(SalaryProfile.is_active.is_(True), SalaryProfile.effective_to.is_not(None)),
        )
    ).all()
    if not profiles:
        raise HTTPException(status_code=409, detail=f"No salary profiles found for {currency} in this period")

    _ensure_payroll_sequence(db, tenant.organization_id)
    run = PayrollRun(
        organization_id=tenant.organization_id,
        run_number=next_sequence_code(db, tenant.organization_id, "payroll"),
        period_id=period.id,
        currency=currency,
        created_by_user_id=tenant.user_id,
    )
    ...
    for profile in sorted(profiles, key=lambda item: item.effective_from, reverse=True):
        if profile.employee_id in seen_employees:
            continue
        seen_employees.add(profile.employee_id)
        allowances = profile.default_allowances or []
        deductions = profile.default_deductions or []
        allowance_total = _component_total(allowances)
        deduction_total = _component_total(deductions)
        gross = _money(profile.base_salary + allowance_total)
        net = _money(gross - deduction_total)
        if net < 0:
            raise HTTPException(status_code=409, detail="Payroll entry cannot have negative net pay")
        db.add(
            PayrollEntry(
                organization_id=tenant.organization_id,
                run_id=run.id,
...
def update_entry(run_id: str, entry_id: str, payload: PayrollEntryUpdate, request: Request, db: DbSession, tenant: PayrollManager):
    run = db.scalar(
        select(PayrollRun)
        .where(PayrollRun.id == run_id, PayrollRun.organization_id == tenant.organization_id)
        .with_for_update()
    )
    if run is None:
        raise HTTPException(status_code=404, detail="Payroll run not found")
    if run.status != "draft":
        raise HTTPException(status_code=409, detail="Only draft payroll can be edited")
    entry = db.scalar(
        select(PayrollEntry)
        .where(
    ...
    if payload.allowances is not None:
        entry.allowances = _component_dicts(payload.allowances)
        entry.allowance_total = _component_total(entry.allowances)
    if payload.deductions is not None:
        entry.deductions = _component_dicts(payload.deductions)
        entry.deduction_total = _component_total(entry.deductions)
    if payload.tax_amount is not None:
        entry.tax_amount = _money(payload.tax_amount)
    ...
    record_activity(
        db,
        action="payroll.entry.updated",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="payroll_entry",
        entity_id=entry.id,
        after={"net_pay": str(entry.net_pay), "tax_amount": str(entry.tax_amount)},
        request=request,
    )
    db.commit()
    db.refresh(run)
...
def approve_run(run_id: str, request: Request, db: DbSession, tenant: PayrollManager):
    ...
    record_activity(
        ...
        entity_type="payroll_run",
        entity_id=run.id,
        after={"status": run.status, "net_total": str(run.net_total)},
        request=request,
    )
    db.commit()
    db.refresh(run)
...
def pay_run(run_id: str, payload: PayrollPayRequest, request: Request, db: DbSession, tenant: PayrollManager):
    ...
    if run.status != "approved":
        raise HTTPException(status_code=409, detail="Only approved payroll can be paid")
    ...
    db.add(
        FinancialTransaction(
            ...
            description=f"Payroll {run.run_number} · {period.name}",
            created_by_user_id=tenant.user_id,
        )
    )
    run.status = "paid"
    run.paid_account_id = account.id
    ...
    record_activity(
        ...
        entity_type="payroll_run",
        entity_id=run.id,
        after={"status": "paid", "account_id": account.id, "amount": str(run.net_total), "currency": run.currency},
        request=request,
    )
    db.commit()
    db.refresh(run)
