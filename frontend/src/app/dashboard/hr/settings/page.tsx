...
export default function HRSettingsPage() {
  ...
  {modal === "shift" ? <Modal title="Add work schedule" onClose={() => setModal(null)}><form onSubmit={addShift} className="space-y-4"><Field label="Schedule name" name="name" required placeholder="Standard office hours" /><div className="grid gap-4 sm:grid-cols-2"><Field label="Starts" name="start_time" type="time" required /><Field label="Ends" name="end_time" type="time" required /></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Break (minutes)" name="break_minutes" type="number" defaultValue="60" /><Field label="Grace time (minutes)" name="grace_minutes" type="number" defaultValue="10" /></div><div><p className="text-sm font-medium">Weekly days off</p><div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-4">{days.map((day, index) => <label key={day} className="flex items-center gap-2 rounded-lg border px-3 py-2 text-xs"><input type="checkbox" name="weekly_off_days" value={index} defaultChecked={index >= 5} />{day}</label>)}</div></div><p className="rounded-xl bg-neutral-50 p-3 text-xs leading-5 text-neutral-500">Overnight schedules are supported. Attendance overtime uses the assigned schedule; without one, the system keeps the existing 8-hour fallback.</p><Submit saving={saving} label="Create schedule" /></form></Modal> : null}
