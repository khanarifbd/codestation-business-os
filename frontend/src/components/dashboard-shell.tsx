...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-neutral-100 text-neutral-950 lg:flex">
    <aside className="hidden h-screen w-60 shrink-0 border-r border-neutral-200 bg-white p-4 lg:sticky lg:top-0 lg:flex lg:flex-col">
      ...
      <nav className="mt-3 space-y-1 overflow-y-auto pb-4">{navigation.map(({ label, icon: Icon, href }) => {
        let active = false;
        if (href === "/dashboard") active = pathname === "/dashboard";
        else if (href === "/dashboard/finance") active = pathname === href || (pathname.startsWith(`${href}/`) && !pathname.startsWith("/dashboard/finance/transfers"));
        else if (href) active = pathname === href || pathname.startsWith(`${href}/`);
        const className = `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm transition ${active ? "bg-neutral-950 font-medium text-white" : href ? "text-neutral-600 hover:bg-neutral-100 hover:text-neutral-950" : "cursor-default text-neutral-300"}`;
        return href ? <Link key={label} href={href} className={className}><Icon className="size-4" />{label}</Link> : <div key={label} className={className} title="Coming soon"><Icon className="size-4" />{label}</div>;
      })}</nav>
      <button type="button" onClick={logout} className="mt-auto flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-neutral-500 hover:bg-neutral-100 hover:text-neutral-950"><LogOut className="size-4" />Sign out</button>
    </aside>
