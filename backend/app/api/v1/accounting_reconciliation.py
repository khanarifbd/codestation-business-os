...
from app.schemas.accounting_reconciliation import (
    ReconciliationDetailRead,
    ReconciliationMetaRead,
    ReconciliationRead,
)
...
def account_or_404(db: DbSession, org: str, account_id: str) -> FinancialAccount:
    row = db.scalar(
        select(FinancialAccount).where(
            ...
            FinancialAccount.organization_id == org,
        )
    )
    if row is None:
...
def reconciliation_or_404(db: DbSession, org: str, reconciliation_id: str) -> BankReconciliation:
    row = db.scalar(
        select(BankReconciliation).where(
            ...
            BankReconciliation.organization_id == org,
        )
    )
    if row is None:
...
def cleared_balance(db: DbSession, row: BankReconciliation) -> Decimal:
    ...
    return money(
        ...
        + sum((tx_effect(direction, amount) for direction, amount in selected), Decimal("0"))
    )
...
def row_json(db: DbSession, row: BankReconciliation) -> dict:
    ...
    account = account_or_404(db, row.organization_id, row.account_id)
    count = db.scalar(
        select(func.count(BankReconciliationItem.id)).where(
            BankReconciliationItem.organization_id == row.organization_id,
            BankReconciliationItem.reconciliation_id == row.id,
        )
    ) or 0
    return {
        "id": row.id,
...
@router.get("/meta", response_model=ReconciliationMetaRead)
    ...
    for account in accounts:
        last = db.scalar(
            ...
            .where(
                ...
                BankReconciliation.account_id == account.id,
                BankReconciliation.status == "finalized",
            )
            .order_by(BankReconciliation.statement_end_date.desc())
            .limit(1)
        )
        result.append(
            {
                ...
                "currency": account.currency,
                "last_reconciled_date": last.statement_end_date if last else None,
                "last_statement_balance": last.statement_ending_balance if last else None,
            }
        )
...
def list_reconciliations(db: DbSession, tenant: Viewer):
    rows = db.scalars(
        select(BankReconciliation)
        .where(BankReconciliation.organization_id == tenant.organization_id)
        .order_by(BankReconciliation.statement_end_date.desc(), BankReconciliation.created_at.desc())
        .limit(200)
    ).all()
...
def create_reconciliation(
    ...
    start = payload.statement_start_date or (last.statement_end_date + timedelta(days=1) if last else None)
    if last and payload.statement_end_date <= last.statement_end_date:
        raise HTTPException(
            status_code=409,
            detail=f"Statement end date must be after the last reconciliation ({last.statement_end_date.isoformat()})",
        )
    ...
    row = BankReconciliation(
        ...
        statement_ending_balance=money(payload.statement_ending_balance),
        cleared_book_balance=money(account.opening_balance),
...
def detail(reconciliation_id: str, db: DbSession, tenant: Viewer):
    ...
    selected_ids = set(
        db.scalars(
            select(BankReconciliationItem.financial_transaction_id).where(
                BankReconciliationItem.organization_id == tenant.organization_id,
                BankReconciliationItem.reconciliation_id == row.id,
            )
        ).all()
    )
    used_subquery = select(BankReconciliationItem.financial_transaction_id).where(
        BankReconciliationItem.organization_id == tenant.organization_id,
        BankReconciliationItem.reconciliation_id != row.id,
    )
    transactions = db.scalars(
        ...
        .where(
            ...
            FinancialTransaction.transaction_date <= row.statement_end_date,
            ~FinancialTransaction.id.in_(used_subquery),
        )
        .order_by(FinancialTransaction.transaction_date.asc(), FinancialTransaction.created_at.asc())
        .limit(2000)
    ).all()
    ...
    data["transactions"] = [
        {
            ...
            "selected": tx.id in selected_ids,
        }
...
@router.post(
    "/{reconciliation_id}/transactions/{transaction_id}",
...
def select_transaction(
    ...
    row = reconciliation_or_404(db, tenant.organization_id, reconciliation_id)
    if row.status != "draft":
        raise HTTPException(status_code=409, detail="Finalized reconciliation cannot be changed")
    tx = db.scalar(
    ...
    if tx is None:
        raise HTTPException(status_code=404, detail="Eligible account transaction not found")
    existing = db.scalar(
        select(BankReconciliationItem).where(
            BankReconciliationItem.organization_id == tenant.organization_id,
            BankReconciliationItem.financial_transaction_id == tx.id,
        )
    )
    if existing:
    ...
    record_activity(
        db,
        action="accounting.reconciliation.match",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="bank_reconciliation",
        entity_id=row.id,
        after={"transaction_id": tx.id, "difference": row.difference},
        request=request,
    )
    db.commit()
    return row_json(db, row)
...
def unselect_transaction(
    ...
    item = db.scalar(
        select(BankReconciliationItem).where(
            BankReconciliationItem.organization_id == tenant.organization_id,
            BankReconciliationItem.reconciliation_id == row.id,
            BankReconciliationItem.financial_transaction_id == transaction_id,
        )
    )
    if item is None:
        return None
    ...
    record_activity(
        db,
        action="accounting.reconciliation.unmatch",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="bank_reconciliation",
        entity_id=row.id,
        after={"transaction_id": transaction_id, "difference": row.difference},
        request=request,
    )
    db.commit()
    return None
...
@router.post("/{reconciliation_id}/finalize", response_model=ReconciliationRead)
def finalize(reconciliation_id: str, request: Request, db: DbSession, tenant: Manager):
    ...
    if row.difference != Decimal("0.00"):
        raise HTTPException(
            status_code=409,
            detail=f"Reconciliation difference must be 0.00 before finalizing. Current difference: {row.difference}",
        )
    later = db.scalar(
        select(BankReconciliation.id).where(
            BankReconciliation.organization_id == tenant.organization_id,
            BankReconciliation.account_id == row.account_id,
            BankReconciliation.status == "finalized",
            BankReconciliation.statement_end_date >= row.statement_end_date,
        )
    )
    if later:
        raise HTTPException(status_code=409, detail="A later reconciliation already exists for this account")
    row.status = "finalized"
    row.finalized_by_user_id = tenant.user_id
    row.finalized_at = datetime.now(timezone.utc)
    record_activity(
        db,
        action="accounting.reconciliation.finalize",
        scope="tenant",
        actor_user_id=tenant.user_id,
        organization_id=tenant.organization_id,
        entity_type="bank_reconciliation",
