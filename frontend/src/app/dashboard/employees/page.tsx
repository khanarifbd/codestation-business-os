...
const tabs: Array<[Tab, string]> = [["employees", "Employees"], ["invitations", "Invitations"], ["structure", "Departments & Designations"], ["roles", "Roles & Permissions"]];
    ...
    <main className="min-h-screen bg-neutral-100 p-5 sm:p-8 lg:p-10">
      <div className="mx-auto max-w-[1500px]">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <button onClick={() => setTab("invitations")} className="flex h-11 items-center gap-2 rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white">
        ...
        <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ["Employees", bundle.summary.total_employees, UsersRound],
            ["Active", bundle.summary.active_employees, BadgeCheck],
            ["Suspended", bundle.summary.suspended_memberships, UserRoundCog],
            ["Pending invites", bundle.summary.pending_invitations, MailPlus],
          ].map(([label, count, Icon]) => (
            <article key={String(label)} className="rounded-2xl border bg-white p-5 shadow-sm">
              <div className="flex items-center justify-between"><p className="text-sm text-neutral-500">{String(label)}</p><Icon className="size-4 text-neutral-400" /></div>
              <p className="mt-4 text-3xl font-semibold">{String(count)}</p>
            </article>
          ))}
        </div>
        ...
        <div className="mt-5 overflow-x-auto rounded-2xl border bg-white p-2 shadow-sm">
          <div className="flex min-w-max gap-1">
            {tabs.map(([id, label]) => (
              <button key={id} onClick={() => { setTab(id); setError(null); setMessage(null); }} className={`rounded-xl px-4 py-2.5 text-sm font-medium ${tab === id ? "bg-neutral-950 text-white" : "text-neutral-600 hover:bg-neutral-100"}`}>{label}</button>
            ))}
          </div>
        </div>
          ...
          <section className="mt-5 overflow-hidden rounded-2xl border bg-white shadow-sm">
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] text-left text-sm">
                <thead className="bg-neutral-50 text-xs uppercase tracking-wide text-neutral-400"><tr>
                  <th className="px-6 py-3 font-medium">Employee</th><th className="px-4 py-3 font-medium">Role</th><th className="px-4 py-3 font-medium">Department</th><th className="px-4 py-3 font-medium">Designation</th><th className="px-4 py-3 font-medium">Status</th><th className="px-6 py-3 text-right font-medium">Action</th>
                </tr></thead>
                <tbody className="divide-y">
                  {bundle.employees.map((employee) => <tr key={employee.id}>
                    <td className="px-6 py-4"><p className="font-medium">{employee.full_name}</p><p className="mt-1 text-xs text-neutral-400">{employee.employee_code} · {employee.login_email}</p></td>
                    <td className="px-4 py-4">{employee.role_name}</td><td className="px-4 py-4">{employee.department_name ?? "—"}</td><td className="px-4 py-4">{employee.designation_name ?? "—"}</td>
                    <td className="px-4 py-4"><span className="rounded-full border px-2.5 py-1 text-xs capitalize">{employee.membership_status}</span></td>
                    <td className="px-6 py-4 text-right"><button onClick={() => setEditingEmployee(employee)} className="inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-semibold hover:bg-neutral-50"><Pencil className="size-3.5" /> Edit</button></td>
                  </tr>)}
                </tbody>
              </table>
            </div>
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[0.75fr_1.25fr]">
            <section className="rounded-2xl border bg-white p-6 shadow-sm">
              ...
              <form onSubmit={(event) => {
                ...
                <Select label="Role" name="role_id" options={activeRoles.map((r) => [r.id, r.name])} required />
                <Select label="Department" name="department_id" options={activeDepartments.map((d) => [d.id, d.name])} empty="No department" />
                <Select label="Designation" name="designation_id" options={activeDesignations.map((d) => [d.id, d.name])} empty="No designation" />
            ...
            <section className="overflow-hidden rounded-2xl border bg-white shadow-sm">
              ...
              <div className="divide-y">{bundle.invitations.map((invite) => <div key={invite.id} className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-medium">{invite.full_name}</p><p className="mt-1 text-sm text-neutral-500">{invite.email} · {invite.role_name} · {invite.employee_code}</p><p className="mt-1 text-xs text-neutral-400">Expires {new Date(invite.expires_at).toLocaleString()}</p></div><div className="flex items-center gap-2"><span className="rounded-full border px-2.5 py-1 text-xs capitalize">{invite.status}</span>{invite.status === "pending" ? <button disabled={saving} onClick={() => void run(() => api(`/invitations/${invite.id}/revoke`, "POST").then(() => undefined), "Invitation revoked")} className="rounded-lg border px-3 py-2 text-xs font-semibold">Revoke</button> : null}</div></div>)}</div>
            </section>
          </div>
        ) : null}
        ...
        {tab === "roles" ? (
          <div className="mt-5 grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
            <section className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center gap-2"><ShieldCheck className="size-4" /><h2 className="font-semibold">Create custom role</h2></div><p className="mt-1 text-sm text-neutral-500">Choose only the permissions this role needs.</p>
              <form onSubmit={(event) => { event.preventDefault(); const formElement = event.currentTarget; const form = new FormData(formElement); void run(async () => { await api("/roles", "POST", { name: value(form, "name"), description: value(form, "description"), permissions: bundle.permission_catalog.filter((permission) => form.get(`permission:${permission}`) === "on") }); formElement.reset(); }, "Role created"); }} className="mt-5 space-y-4"><Field label="Role name" name="name" required /><Field label="Description" name="description" />
            ...
            <section className="overflow-hidden rounded-2xl border bg-white shadow-sm"><div className="border-b px-6 py-5"><h2 className="font-semibold">Company roles</h2><p className="mt-1 text-sm text-neutral-500">Built-in Admin/User roles are protected.</p></div><div className="divide-y">{bundle.roles.map((role) => <div key={role.id} className="p-5"><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{role.name}</p><p className="mt-1 text-xs text-neutral-400">{role.is_system ? "Built-in role" : "Custom role"}</p></div><span className="rounded-full border px-2.5 py-1 text-xs">{role.is_active ? "Active" : "Inactive"}</span></div><p className="mt-3 text-sm text-neutral-500">{role.description ?? "No description"}</p><div className="mt-3 flex flex-wrap gap-1.5">{role.permissions.map((permission) => <span key={permission} className="rounded-md bg-neutral-100 px-2 py-1 text-xs text-neutral-600">{permission}</span>)}</div></div>)}</div></section>
          </div>
        ) : null}
      </div>

      {editingEmployee ? <EmployeeModal employee={editingEmployee} employees={bundle.employees} roles={activeRoles} departments={activeDepartments} designations={activeDesignations} saving={saving} onClose={() => setEditingEmployee(null)} onSave={(form) => void run(async () => { await api(`/employees/${editingEmployee.id}`, "PATCH", {
      ...
      }); setEditingEmployee(null); }, "Employee updated")} /> : null}
    </main>
  );
}

function Field({ label, name, type = "text", required = false, placeholder, defaultValue }: { label: string; name: string; type?: string; required?: boolean; placeholder?: string; defaultValue?: string | null }) { return <label className="block text-sm font-medium">{label}<input name={name} type={type} required={required} placeholder={placeholder} defaultValue={defaultValue ?? ""} className={inputClass} /></label>; }
function Select({ label, name, options, required = false, empty, defaultValue }: { label: string; name: string; options: Array<[string, string]>; required?: boolean; empty?: string; defaultValue?: string | null }) { return <label className="block text-sm font-medium">{label}<select name={name} required={required} defaultValue={defaultValue ?? ""} className={inputClass}>{empty ? <option value="">{empty}</option> : null}{!required && !empty ? <option value="">—</option> : null}{options.map(([id, text]) => <option key={id} value={id}>{text}</option>)}</select></label>; }

function StructureCard({ title, icon: Icon, items, saving, onCreate, onToggle }: { title: string; icon: typeof Building2; items: Array<Department | Designation>; saving: boolean; onCreate: (form: FormData) => Promise<void>; onToggle: (item: Department | Designation) => Promise<void> }) {
  return <section className="rounded-2xl border bg-white p-6 shadow-sm"><div className="flex items-center gap-2"><Icon className="size-4" /><h2 className="font-semibold">{title}</h2></div><form onSubmit={(event) => { event.preventDefault(); const element = event.currentTarget; void onCreate(new FormData(element)).then(() => element.reset()); }} className="mt-5 grid gap-3 sm:grid-cols-[1fr_120px_auto]"><Field label="Name" name="name" required /><Field label="Code" name="code" /><button disabled={saving} className="mt-auto flex h-11 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white"><Plus className="size-4" /> Add</button><div className="sm:col-span-3"><Field label="Description" name="description" /></div></form><div className="mt-5 divide-y rounded-xl border">{items.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 p-4"><div><p className="font-medium">{item.name}</p><p className="mt-1 text-xs text-neutral-400">{item.code ?? "No code"}{item.description ? ` · ${item.description}` : ""}</p></div><button disabled={saving} onClick={() => void onToggle(item)} className="rounded-lg border px-3 py-2 text-xs font-semibold">{item.is_active ? "Deactivate" : "Activate"}</button></div>)}</div></section>;
}
