...
class AccountingIncomeWithFeeCreate(BaseModel):
    ...
    def validate_income_with_fee(self):
        ...
        if self.fee_amount > 0 and not self.fee_category_ledger_account_id:
            raise ValueError("fee_category_ledger_account_id is required when fee_amount is greater than zero")
        return self
