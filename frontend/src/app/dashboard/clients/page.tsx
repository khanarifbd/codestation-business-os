...
export default function ClientsPage() {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-neutral-100 p-3 sm:p-8 lg:p-10"><div className="mx-auto max-w-[1500px]">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><p className="text-sm font-medium text-neutral-500">Business relationships</p><h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">Clients</h1><p className="mt-2 text-sm leading-6 text-neutral-500">Master client records shared by quotations, orders, projects, invoices and payments.</p></div><button onClick={() => setCreateOpen(true)} className="flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white sm:w-auto"><Plus className="size-4" /> Add client</button></header>
    <div className="mt-5 grid grid-cols-3 gap-2 sm:mt-7 sm:gap-4"><Stat label="Total" value={summary?.total ?? 0} icon={Building2} /><Stat label="Active" value={summary?.active ?? 0} icon={UserRound} /><Stat label="Inactive" value={summary?.inactive ?? 0} icon={UserRound} /></div>
    ...
    <section className="mt-4 overflow-hidden rounded-2xl border bg-white shadow-sm sm:mt-5"><div className="grid grid-cols-2 gap-2 border-b p-3 sm:grid-cols-[minmax(260px,1fr)_220px_auto] sm:gap-3 sm:p-5"><form onSubmit={(e) => { e.preventDefault(); setSearch(searchDraft.trim()); }} className="relative col-span-2 sm:col-span-1"><Search className="absolute left-3 top-3.5 size-4 text-neutral-400" /><input value={searchDraft} onChange={(e) => setSearchDraft(e.target.value)} placeholder="Search clients..." className="h-11 w-full rounded-xl border pl-9 pr-3 text-sm outline-none focus:border-neutral-500" /></form><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-11 min-w-0 rounded-xl border bg-white px-3 text-sm"><option value="">All statuses</option><option value="active">Active</option><option value="inactive">Inactive</option></select><button onClick={() => { setSearchDraft(""); setSearch(""); setStatusFilter(""); }} className="h-11 rounded-xl border px-3 text-sm font-medium sm:px-4">Reset</button></div>
      ...
      {loading ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="size-6 animate-spin text-neutral-400" /></div> : clients.length === 0 ? <div className="px-6 py-16 text-center sm:py-20"><Building2 className="mx-auto size-8 text-neutral-300" /><h2 className="mt-4 font-semibold">No clients found</h2><p className="mt-1 text-sm text-neutral-500">Convert a CRM lead or create a client directly.</p></div> : <><div className="divide-y md:hidden">{clients.map((client) => <article key={client.id} className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-semibold">{client.display_name}</p><p className="mt-1 text-xs capitalize text-neutral-400">{client.client_code} · {client.client_type}</p></div><Status status={client.status} /></div><div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-3 text-xs"><div className="min-w-0"><p className="text-neutral-400">Contact</p><p className="mt-1 break-words font-medium text-neutral-700">{client.contact_name ?? client.email ?? client.phone ?? "No contact"}</p>{client.contact_name && (client.email || client.phone) ? <p className="mt-0.5 break-all text-neutral-400">{client.email ?? client.phone}</p> : null}</div><div><p className="text-neutral-400">Country / currency</p><p className="mt-1 font-medium text-neutral-700">{client.country_code ?? "—"}{client.currency ? ` · ${client.currency}` : ""}</p></div><div className="min-w-0"><p className="text-neutral-400">Assigned</p><p className="mt-1 break-words font-medium text-neutral-700">{client.assigned_employee_name ?? "Unassigned"}</p></div><div><p className="text-neutral-400">Portal access</p><div className="mt-1"><PortalAccessToggle status={portalAccess[client.id]} canManage={canManagePortalAccess} working={portalWorkingId === client.id} onToggle={() => void togglePortalAccess(client)} /></div></div></div><div className="mt-4 grid grid-cols-2 gap-2"><button onClick={() => router.push(`/dashboard/clients/${encodeURIComponent(client.id)}`)} className="h-10 rounded-xl bg-neutral-950 px-3 text-xs font-semibold text-white">View</button><button onClick={() => void openClient(client.id)} className="h-10 rounded-xl border px-3 text-xs font-semibold">Edit</button></div></article>)}</div><div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[1120px] text-left text-sm"><thead className="bg-neutral-50 text-xs uppercase tracking-wide text-neutral-400"><tr><th className="px-6 py-3 font-medium">Client</th><th className="px-4 py-3 font-medium">Contact</th><th className="px-4 py-3 font-medium">Country</th><th className="px-4 py-3 font-medium">Assigned</th><th className="px-4 py-3 font-medium">Status</th><th className="px-4 py-3 font-medium">Portal access</th><th className="px-6 py-3 text-right font-medium">Action</th></tr></thead><tbody className="divide-y">{clients.map((client) => <tr key={client.id} className="hover:bg-neutral-50/70"><td className="px-6 py-4"><p className="font-medium">{client.display_name}</p><p className="mt-1 text-xs text-neutral-400">{client.client_code} · {client.client_type}</p></td><td className="px-4 py-4"><p>{client.contact_name ?? "—"}</p><p className="mt-1 text-xs text-neutral-400">{client.email ?? client.phone ?? "No contact"}</p></td><td className="px-4 py-4">{client.country_code ?? "—"}{client.currency ? ` · ${client.currency}` : ""}</td><td className="px-4 py-4">{client.assigned_employee_name ?? "Unassigned"}</td><td className="px-4 py-4"><Status status={client.status} /></td><td className="px-4 py-4"><PortalAccessToggle status={portalAccess[client.id]} canManage={canManagePortalAccess} working={portalWorkingId === client.id} onToggle={() => void togglePortalAccess(client)} /></td><td className="px-6 py-4 text-right"><div className="inline-flex gap-2"><button onClick={() => router.push(`/dashboard/clients/${encodeURIComponent(client.id)}`)} className="rounded-lg bg-neutral-950 px-3 py-2 text-xs font-semibold text-white">View</button><button onClick={() => void openClient(client.id)} className="rounded-lg border px-3 py-2 text-xs font-semibold">Edit</button></div></td></tr>)}</tbody></table></div>{nextCursor ? <div className="border-t p-4 text-center"><button disabled={loadingMore} onClick={() => void loadMore()} className="w-full rounded-xl border px-5 py-2.5 text-sm font-semibold disabled:opacity-50 sm:w-auto">{loadingMore ? "Loading..." : "Load more"}</button></div> : null}</>}
    </section>
  </div>
...
function ClientDrawer({ detail, loading, meta, saving, onClose, onSave, onStatus, onQuotation }: { detail: ClientDetail | null; loading: boolean; meta: Meta | null; saving: boolean; onClose: () => void; onSave: (event: FormEvent<HTMLFormElement>) => void; onStatus: () => void; onQuotation: () => void }) {
  return <div className="fixed inset-0 z-50 bg-black/30" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><aside className="ml-auto h-full w-full max-w-3xl overflow-y-auto bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b bg-white px-4 py-4 sm:px-6 sm:py-5"><div className="min-w-0"><p className="text-xs uppercase tracking-wide text-neutral-400">Client master</p><h2 className="mt-1 break-words text-lg font-semibold sm:text-xl">{detail?.display_name ?? "Loading..."}</h2></div><button onClick={onClose} className="flex size-10 shrink-0 items-center justify-center rounded-xl border"><X className="size-4" /></button></div>{loading || !detail || !meta ? <div className="flex min-h-64 items-center justify-center"><Loader2 className="size-6 animate-spin" /></div> : <div className="space-y-5 p-4 sm:space-y-6 sm:p-6"><div className="flex flex-col gap-4 rounded-2xl border p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5"><div className="min-w-0"><p className="text-xs uppercase tracking-wide text-neutral-400">{detail.client_code}</p><div className="mt-2"><Status status={detail.status} /></div>{detail.source_lead_code ? <p className="mt-2 break-words text-xs text-neutral-500">Originated from CRM · {detail.source_lead_code} · {detail.source_lead_status}</p> : <p className="mt-2 break-words text-xs text-neutral-400">Direct client record{detail.acquisition_source_name ? ` · ${detail.acquisition_source_name}` : ""}</p>}</div><div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap"><button disabled={detail.status !== "active"} onClick={onQuotation} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-3 text-xs font-semibold text-white disabled:opacity-30 sm:px-4 sm:text-sm"><FileText className="size-4 shrink-0" /> <span className="truncate">Quotation</span></button><button disabled={saving} onClick={onStatus} className="h-10 rounded-xl border px-3 text-xs font-semibold sm:px-4 sm:text-sm">{detail.status === "active" ? "Deactivate" : "Activate"}</button></div></div><ClientForm meta={meta} detail={detail} saving={saving} onSubmit={onSave} /></div>}</aside></div>;
}
...
function Stat({ label, value, icon: Icon }: { label: string; value: number | string; icon: typeof Building2 }) { return <article className="min-w-0 rounded-2xl border bg-white p-3 shadow-sm sm:p-5"><div className="flex items-center justify-between gap-2"><p className="truncate text-[11px] text-neutral-500 sm:text-sm">{label}</p><Icon className="size-3.5 shrink-0 text-neutral-400 sm:size-4" /></div><p className="mt-3 text-xl font-semibold sm:mt-4 sm:text-2xl">{value}</p></article>; }
function Status({ status }: { status: string }) { return <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs capitalize ${status === "active" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "bg-neutral-50 text-neutral-500"}`}>{status}</span>; }
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/30 p-0 sm:p-4" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><div className="h-[100dvh] max-h-none w-full max-w-4xl overflow-y-auto bg-white shadow-2xl sm:h-auto sm:max-h-[92vh] sm:rounded-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-4 py-4 sm:px-6 sm:py-5"><h2 className="text-lg font-semibold sm:text-xl">{title}</h2><button onClick={onClose} className="flex size-10 items-center justify-center rounded-xl border"><X className="size-4" /></button></div><div className="p-4 sm:p-6">{children}</div></div></div>; }