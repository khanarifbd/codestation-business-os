...
def public_app_endpoint(method: str, path: str):
    public_path = f"{API_PREFIX}{path}"
    matches = [
        route
        for route in app.routes
        if isinstance(route, APIRoute)
        and route.path == public_path
        and method in (route.methods or set())
    ]
    if len(matches) != 1:
        raise AssertionError(f"expected one public {method} {public_path}, found {len(matches)}")
...
def main() -> None:
    ...
    try:
        ...
        for method, path, expected_name in (
            ...
            endpoint = public_app_endpoint(method, path)
