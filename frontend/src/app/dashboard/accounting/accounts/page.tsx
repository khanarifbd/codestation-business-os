...
const accountTypes: Array<{ value: AccountType; label: string; help: string }> = [
  ...
  { value: "credit_card", label: "Credit card", help: "Company credit card account" },
...
export default function AccountsPage() {
  ...
  const totals = useMemo(() => { const result = new Map<string, number>(); for (const item of accounts.filter((account) => account.is_active)) result.set(item.currency, (result.get(item.currency) ?? 0) + Number(item.current_balance)); return [...result.entries()]; }, [accounts]);
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {showForm ? <section className="rounded-2xl border bg-white p-5"><div><h2 className="font-semibold">Add a financial account</h2><p className="mt-1 text-sm text-neutral-500">Choose the account people use in the real world. Accounting ledger mapping happens automatically.</p></div><form onSubmit={submit} className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      <Field label="Opening balance"><input type="number" step="0.01" value={form.opening_balance} onChange={(e) => setForm((v) => ({ ...v, opening_balance: e.target.value }))} className="w-full rounded-xl border px-3 py-2.5" /><p className="mt-1 text-xs text-neutral-400">Money already in this account when you start using Business OS.</p></Field>
