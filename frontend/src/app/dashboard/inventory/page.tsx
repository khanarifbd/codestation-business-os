...
import { ServiceDurationFields } from "@/components/service-duration-fields";
import { ServiceSalesPanel } from "@/components/service-sales-panel";
import { getApiErrorMessage } from "@/lib/api-error";
import { COUNTRY_OPTIONS, CURRENCY_OPTIONS } from "@/lib/company-options";

type Tab = "overview" | "items" | "service_sales" | "suppliers" | "warehouses" | "purchases" | "stock" | "movements";
type Overview = {
...
const emptyItem = (currency = "BDT") => ({ sku: "", barcode: "", name: "", description: "", item_type: "stock_item", category_id: "", unit: "unit", currency, selling_price: "0", standard_cost: "0", reorder_level: "0", tax_code_id: "", track_inventory: true, allow_negative_stock: false, is_active: true, service_duration_months: "" });
...
export default function InventoryPage() {
  ...
  const activeCategoryOptions = useMemo(() => [
    ...
    ...categories.filter((category) => category.is_active).map((category) => ({ value: category.id, label: category.name })),
  ], [categories]);
  ...
  async function submitItem(event: FormEvent) {
    ...
    const duration = itemForm.item_type === "service" && itemForm.service_duration_months !== "" ? Number(itemForm.service_duration_months) : null;
    if (duration !== null && (!Number.isInteger(duration) || duration < 1 || duration > 120)) {
      setError("Service duration must be a whole number between 1 and 120 months.");
      return;
    }
    const { service_duration_months: _duration, ...baseForm } = itemForm;
    const body = { ...baseForm, barcode: baseForm.barcode || null, description: baseForm.description || null, category_id: baseForm.category_id || null, tax_code_id: baseForm.tax_code_id || null, selling_price: Number(baseForm.selling_price), standard_cost: Number(baseForm.standard_cost), reorder_level: Number(baseForm.reorder_level), track_inventory: baseForm.item_type === "stock_item" ? baseForm.track_inventory : false };
    setSaving(true); setError(null); setMessage(null);
    try {
      const saved = await request(editingId ? `/api/inventory/products/${editingId}` : "/api/inventory/products", { method: editingId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  const tabs: { id: Tab; label: string; icon: typeof Boxes }[] = [
    ...
    { id: "service_sales", label: "Service Sales", icon: BarChart3 },
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-[1500px] space-y-6">
      ...
      <section className="min-w-0">{loading ? <Panel><p className="text-sm text-neutral-500">Loading inventory…</p></Panel> : <>
        {tab === "overview" ? <div className="space-y-5">
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <Panel><SectionTitle title="Quick actions" subtitle="Common day-to-day inventory work" /><div className="mt-4 grid gap-3 sm:grid-cols-2"><Quick title="Add a product" text="Physical item you buy, keep in stock and sell." onClick={() => openNewItem("stock_item")} /><Quick title="Add a service" text="One-time or fixed-term service you quote, order and invoice without stock." onClick={() => openNewItem("service")} /><Quick title="Service sales" text="See which services sell most and which fixed terms are active." onClick={() => setTab("service_sales")} /><Quick title="Receive a purchase" text="Increase stock and create the related payable accounting entry." onClick={openPurchase} /><Quick title="Adjust stock" text="Correct damaged, counted or opening stock with a reason." onClick={openAdjustment} /><Quick title="Transfer stock" text="Move existing stock between warehouses without changing total inventory value." onClick={openTransfer} /><Quick title="Add a supplier" text="Save supplier details and default purchasing currency." onClick={openNewSupplier} /></div></Panel>
    ...
    {modal ? <ModalShell title={modalTitle(modal, editingId)} onClose={() => { setModal(null); setEditingId(null); }}>
      {modal === "item" ? <form onSubmit={submitItem} className="grid gap-4 md:grid-cols-2"><Select label="Item type" helpText="Choose Stock product for a physical item whose quantity you track, Non-stock product for something you sell without stock tracking, or Service for work you provide." value={itemForm.item_type} onChange={(value) => setItemForm({ ...itemForm, item_type: value, track_inventory: value === "stock_item", service_duration_months: value === "service" ? itemForm.service_duration_months : "" })} options={[["stock_item", "Stock product"], ["non_stock_item", "Non-stock product"], ["service", "Service"]]} /><Input label="SKU / Code" helpText="A unique, stable internal code used in search, sales documents and reports. Example: PROD-001 or WEB-DEV. Avoid reusing the same code for different items." value={itemForm.sku} onChange={(value) => setItemForm({ ...itemForm, sku: value })} /><Input label="Name" helpText="The customer-facing name shown when this catalog item is selected in quotations, orders and invoices." value={itemForm.name} onChange={(value) => setItemForm({ ...itemForm, name: value })} /><SearchableField label="Category" helpText="Groups similar products or services for faster search and reporting. Example: Tracking Device. Leave Uncategorized if you do not need grouping yet." value={itemForm.category_id} onChange={(value) => setItemForm({ ...itemForm, category_id: value })} options={activeCategoryOptions} placeholder="Uncategorized" searchPlaceholder="Search categories..." /><Input label="Barcode" helpText="Optional barcode or manufacturer code used for scanning or external identification. Leave blank when the item has no barcode." optional value={itemForm.barcode} onChange={(value) => setItemForm({ ...itemForm, barcode: value })} /><Input label="Unit" helpText="How quantity is measured. Use pcs or unit for physical products; hour, day, project or month for services. Enter a unit name, not a quantity." value={itemForm.unit} onChange={(value) => setItemForm({ ...itemForm, unit: value })} /><CurrencyInput label="Currency" helpText="The currency of this item's default selling price. Pick the currency you normally quote or sell this item in; Business OS keeps currencies separate." value={itemForm.currency} onChange={(value) => setItemForm({ ...itemForm, currency: value })} /><Input label="Selling price" helpText="Default price per unit used to prefill quotations, orders and invoices. You can still change the final price on a specific sales document." type="number" value={itemForm.selling_price} onChange={(value) => setItemForm({ ...itemForm, selling_price: value })} />{itemForm.item_type === "service" ? <ServiceDurationFields durationMonths={itemForm.service_duration_months} onChange={(value) => setItemForm({ ...itemForm, service_duration_months: value })} /> : null}{itemForm.item_type === "stock_item" ? <><Input label="Standard unit cost" helpText="Your expected/default acquisition cost for one unit. It helps prefill stock cost when no purchase history exists, but entering a value here does not add stock. Example: if one item costs 350 in the selected currency, enter 350. Receive a purchase or post a stock adjustment to add quantity." type="number" value={itemForm.standard_cost} onChange={(value) => setItemForm({ ...itemForm, standard_cost: value })} /><Input label="Low-stock alert level" helpText="The on-hand quantity at or below which Business OS flags this item as low stock. Example: enter 10 to get an alert at 10 units or fewer. Use 0 if you only want an out-of-stock warning." type="number" value={itemForm.reorder_level} onChange={(value) => setItemForm({ ...itemForm, reorder_level: value })} /></> : null}<Select label="Sales tax" helpText="Default sales tax for this item when used in sales documents. Choose No tax when tax does not apply; the selected tax is snapshotted on the transaction." required={false} value={itemForm.tax_code_id} onChange={(value) => setItemForm({ ...itemForm, tax_code_id: value })} options={[["", "No tax"], ...salesTaxes.map((tax) => [tax.id, `${tax.code} · ${tax.rate}%`])]} /><Input label="Description" helpText="Optional details that help your team and customers understand what is included in the product or service." optional value={itemForm.description} onChange={(value) => setItemForm({ ...itemForm, description: value })} />{editingId ? <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={itemForm.is_active} onChange={(event) => setItemForm({ ...itemForm, is_active: event.target.checked })} />Active item</label> : null}<Save saving={saving} text={editingId ? "Update item" : "Add item"} /></form> : null}

      {modal === "supplier" ? <form onSubmit={submitSupplier} className="grid gap-4 md:grid-cols-2"><Input label="Supplier name" value={supplierForm.name} onChange={(value) => setSupplierForm({ ...supplierForm, name: value })} /><Input label="Contact person" optional value={supplierForm.contact_name} onChange={(value) => setSupplierForm({ ...supplierForm, contact_name: value })} /><Input label="Email" optional type="email" value={supplierForm.email} onChange={(value) => setSupplierForm({ ...supplierForm, email: value })} /><Input label="Phone" optional value={supplierForm.phone} onChange={(value) => setSupplierForm({ ...supplierForm, phone: value })} /><SearchableSelect label="Country" value={supplierForm.country_code} onValueChange={(value) => setSupplierForm({ ...supplierForm, country_code: value })} options={COUNTRY_OPTIONS} placeholder="Select country" searchPlaceholder="Search country or code..." /><CurrencyInput label="Default currency" value={supplierForm.currency} onChange={(value) => setSupplierForm({ ...supplierForm, currency: value })} /><Input label="Tax / VAT ID" optional value={supplierForm.tax_identifier} onChange={(value) => setSupplierForm({ ...supplierForm, tax_identifier: value })} /><Input label="Website" optional value={supplierForm.website} onChange={(value) => setSupplierForm({ ...supplierForm, website: value })} /><Input label="Notes" optional value={supplierForm.notes} onChange={(value) => setSupplierForm({ ...supplierForm, notes: value })} />{editingId ? <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={supplierForm.is_active} onChange={(event) => setSupplierForm({ ...supplierForm, is_active: event.target.checked })} />Active supplier</label> : null}<Save saving={saving} text={editingId ? "Update supplier" : "Add supplier"} /></form> : null}
      ...
      {modal === "purchase" ? <form onSubmit={submitPurchase} className="space-y-5"><div className="grid gap-4 md:grid-cols-2"><SearchableField label="Supplier" helpText="Choose the supplier you purchased from. This links the purchase to the supplier record and payable history." value={purchaseForm.supplier_id} onChange={(value) => { const supplier = suppliers.find((item) => item.id === value); const currency = supplier?.currency || baseCurrency; setPurchaseForm({ ...purchaseForm, supplier_id: value, currency }); setPurchaseLines([{ product_id: "", quantity: "1", unit_cost: "0", tax_code_id: "" }]); }} options={activeSuppliers.map((supplier) => ({ value: supplier.id, label: `${supplier.name} · ${supplier.vendor_code}`, keywords: `${supplier.name} ${supplier.vendor_code} ${supplier.contact_name ?? ""} ${supplier.email ?? ""}` }))} placeholder="Select supplier" searchPlaceholder="Search supplier name or code..." /><SearchableField label="Receive into warehouse" helpText="Choose the warehouse that will receive the stock. The received quantity and inventory value will increase in this location." value={purchaseForm.warehouse_id} onChange={(value) => setPurchaseForm({ ...purchaseForm, warehouse_id: value })} options={activeWarehouses.map((warehouse) => ({ value: warehouse.id, label: `${warehouse.name} · ${warehouse.code}`, keywords: `${warehouse.name} ${warehouse.code} ${warehouse.address ?? ""}` }))} placeholder="Select warehouse" searchPlaceholder="Search warehouse name or code..." /><Input label="Purchase date" helpText="The date the goods were received or the purchase was recorded. This date is used for inventory movement and related accounting timing." type="date" value={purchaseForm.receipt_date} onChange={(value) => setPurchaseForm({ ...purchaseForm, receipt_date: value })} /><CurrencyInput label="Purchase currency" helpText="The currency shown on the supplier purchase. Product choices are filtered to stock items using this currency, and Business OS keeps different currencies separate." value={purchaseForm.currency} onChange={(value) => { setPurchaseForm({ ...purchaseForm, currency: value }); setPurchaseLines([{ product_id: "", quantity: "1", unit_cost: "0", tax_code_id: "" }]); }} /><Input label="Supplier invoice / reference" helpText="Optional supplier invoice number, bill number, purchase reference or memo used for tracking and reconciliation." optional value={purchaseForm.reference} onChange={(value) => setPurchaseForm({ ...purchaseForm, reference: value })} /><Input label="Notes" helpText="Optional internal note about this purchase. Use it for delivery details, special terms or other context your team may need later." optional value={purchaseForm.notes} onChange={(value) => setPurchaseForm({ ...purchaseForm, notes: value })} /></div><div><div className="mb-3 flex items-center justify-between"><div><h3 className="font-semibold">Products received</h3><p className="mt-1 text-xs text-neutral-500">Only stock products using {purchaseForm.currency} are shown.</p></div><button type="button" onClick={() => setPurchaseLines([...purchaseLines, { product_id: "", quantity: "1", unit_cost: "0", tax_code_id: "" }])} className="rounded-lg border px-3 py-2 text-xs font-semibold">+ Add line</button></div>{purchaseProducts.length === 0 ? <div className="mb-3 rounded-xl bg-amber-50 px-4 py-3 text-sm text-amber-800">No active stock product uses {purchaseForm.currency}. Change the purchase currency or add a matching product.</div> : null}<div className="space-y-3">{purchaseLines.map((line, index) => <div key={index} className="grid gap-3 rounded-xl border p-3 md:grid-cols-[2fr_0.7fr_1fr_1fr_auto]"><SearchableField label="Product" helpText="Choose the stock product being received. Only active stock-tracked products matching the purchase currency are available." value={line.product_id} onChange={(value) => { const product = purchaseProducts.find((item) => item.id === value); const next = [...purchaseLines]; next[index] = { ...line, product_id: value, unit_cost: product ? String(product.last_purchase_cost || product.standard_cost || 0) : line.unit_cost }; setPurchaseLines(next); }} options={purchaseProducts.map((product) => ({ value: product.id, label: `${product.sku} · ${product.name}`, keywords: `${product.sku} ${product.name} ${product.category_name ?? ""}` }))} placeholder="Select product" searchPlaceholder="Search product, SKU or category..." /><Input label="Qty" helpText="Enter the quantity actually received into stock for this product line." type="number" value={line.quantity} onChange={(value) => { const next = [...purchaseLines]; next[index] = { ...line, quantity: value }; setPurchaseLines(next); }} /><Input label="Unit cost" helpText="Enter the supplier cost for one unit in the selected purchase currency. This contributes to inventory valuation and cost history." type="number" value={line.unit_cost} onChange={(value) => { const next = [...purchaseLines]; next[index] = { ...line, unit_cost: value }; setPurchaseLines(next); }} /><Select label="Tax" helpText="Choose the purchase tax applied to this line. Select No tax when none applies; tax affects the purchase total and related accounting." required={false} value={line.tax_code_id} onChange={(value) => { const next = [...purchaseLines]; next[index] = { ...line, tax_code_id: value }; setPurchaseLines(next); }} options={[["", "No tax"], ...purchaseTaxes.map((tax) => [tax.id, `${tax.code} · ${tax.rate}%`])]} /><button type="button" disabled={purchaseLines.length === 1} onClick={() => setPurchaseLines(purchaseLines.filter((_, itemIndex) => itemIndex !== index))} className="self-end rounded-lg border px-3 py-2.5 text-xs disabled:opacity-30">Remove</button></div>)}</div></div><div className="grid gap-3 rounded-xl bg-neutral-50 p-4 sm:grid-cols-3"><Total label="Subtotal" value={money(purchaseTotals.subtotal, purchaseForm.currency)} /><Total label="Estimated tax" value={money(purchaseTotals.tax, purchaseForm.currency)} /><Total label="Estimated total" value={money(purchaseTotals.total, purchaseForm.currency)} strong /></div><Save saving={saving} text="Receive purchase & update stock" /></form> : null}
...
function serviceDurationLabel(months: number | null) { return months == null ? "One-time" : `${months} month${months === 1 ? "" : "s"}`; }
...
function FieldLabel({ label, optional = false, helpText }: { label: string; optional?: boolean; helpText?: string }) { return <div className="flex min-h-5 items-center gap-1.5"><span className="font-medium">{label}{optional ? <span className="ml-1 font-normal text-neutral-400">(optional)</span> : null}</span>{helpText ? <span className="group relative inline-flex"><button type="button" aria-label={`About ${label}`} className="inline-flex size-5 items-center justify-center rounded-full text-neutral-400 transition hover:bg-neutral-100 hover:text-neutral-700 focus:bg-neutral-100 focus:text-neutral-700 focus:outline-none"><Info className="size-3.5" /></button><span role="tooltip" className="pointer-events-none absolute left-0 top-full z-[80] mt-2 hidden w-72 max-w-[calc(100vw-2rem)] rounded-xl bg-neutral-950 px-3 py-2.5 text-xs font-normal leading-5 text-white shadow-xl group-hover:block group-focus-within:block">{helpText}</span></span> : null}</div>; }
function Input({ label, value, onChange, type = "text", optional = false, helpText }: { label: string; value: string; onChange: (value: string) => void; type?: string; optional?: boolean; helpText?: string }) { return <div className="grid gap-1.5 text-sm"><FieldLabel label={label} optional={optional} helpText={helpText} /><input aria-label={label} required={!optional} type={type} step={type === "number" ? "any" : undefined} value={value} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl border px-3 outline-none focus:border-neutral-700" /></div>; }
function SearchableField({ label, value, onChange, options, placeholder, searchPlaceholder, helpText }: { label: string; value: string; onChange: (value: string) => void; options: { value: string; label: string; keywords?: string }[]; placeholder: string; searchPlaceholder: string; helpText?: string }) { return <div className="grid gap-1.5 text-sm"><FieldLabel label={label} helpText={helpText} /><SearchableSelect value={value} onValueChange={onChange} options={options} placeholder={placeholder} searchPlaceholder={searchPlaceholder} /></div>; }
function CurrencyInput({ label, value, onChange, helpText }: { label: string; value: string; onChange: (value: string) => void; helpText?: string }) { return <div className="grid gap-1.5 text-sm"><FieldLabel label={label} helpText={helpText} /><SearchableSelect value={value} onValueChange={onChange} options={CURRENCY_OPTIONS} placeholder="Select currency" searchPlaceholder="Search currency or code..." clearable={false} /></div>; }
function Select({ label, value, onChange, options, required = true, helpText }: { label: string; value: string; onChange: (value: string) => void; options: string[][]; required?: boolean; helpText?: string }) { return <div className="grid gap-1.5 text-sm"><FieldLabel label={label} helpText={helpText} /><select aria-label={label} required={required} value={value} onChange={(event) => onChange(event.target.value)} className="h-11 rounded-xl border bg-white px-3">{options.map(([optionValue, optionLabel]) => <option key={optionValue} value={optionValue}>{optionLabel}</option>)}</select></div>; }
