...
const blankBill = () => ({ supplier_name: "", bill_date: today(), due_date: "", currency: "BDT", amount: "", expense_ledger_account_id: "", description: "", reference: "", notes: "" });
const blankPayment = () => ({ financial_account_id: "", payment_date: today(), amount: "", reference: "", notes: "" });
...
export default function PayablesPage() {
  ...
  const billDirty = mode === "bill" && Boolean(billForm.supplier_name || billForm.amount || billForm.expense_ledger_account_id || billForm.description || billForm.reference || billForm.notes || billForm.due_date);
  const paymentDirty = mode === "pay" && Boolean(payForm.financial_account_id || payForm.amount || payForm.reference || payForm.notes);
  ...
  function closeMode() {
    ...
    setMode(null); setConfirmMode(null);
    if (mode === "bill") setBillForm(blankBill()); else setPayForm(blankPayment());
  }

  function reviewBill(event: FormEvent) { event.preventDefault(); setError(null); setConfirmMode("bill"); }
  async function postBill() {
    ...
    try {
      ...
      setBillForm(blankBill()); setConfirmMode(null); setMode(null); await load(); setSelected(payload);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not record vendor bill"); setConfirmMode(null); }
    finally { setSaving(false); }
  }

  function openPayment(bill: Bill) {
    setSelected(bill); setPayForm({ financial_account_id: "", payment_date: today(), amount: bill.balance_due, reference: "", notes: "" }); setConfirmMode(null); setMode("pay");
  }

  function reviewPayment(event: FormEvent) { event.preventDefault(); if (!selected) return; setError(null); setConfirmMode("payment"); }
  async function postPayment() {
    if (!selected) return; const billId = selected.id;
    setSaving(true); setError(null);
    try {
      ...
      setPayForm(blankPayment()); setConfirmMode(null); setMode(null); await load(); await loadPayments(billId);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not record vendor payment"); setConfirmMode(null); }
    finally { setSaving(false); }
  }

  const billConfirmation = [
    { label: "Supplier", value: billForm.supplier_name || "—" },
    { label: "What is this bill for?", value: billForm.description || "—" },
    { label: "Expense category", value: selectedCategory?.name || "—" },
    { label: "Bill amount", value: money(billForm.amount || 0, billForm.currency), emphasis: true },
    { label: "Bill date", value: billForm.bill_date },
    { label: "Due date", value: billForm.due_date || "No due date" },
    ...(billForm.reference ? [{ label: "Reference", value: billForm.reference }] : []),
  ];
  const paymentConfirmation = selected ? [
    ...
    { label: "Payment amount", value: money(payForm.amount || 0, selected.currency), emphasis: true },
    { label: "Pay from", value: selectedPaymentAccount?.name || "—" },
    { label: "Account balance", value: selectedPaymentAccount ? money(selectedPaymentAccount.current_balance, selectedPaymentAccount.currency) : "—" },
    { label: "Payment date", value: payForm.payment_date },
    ...(payForm.reference ? [{ label: "Reference", value: payForm.reference }] : []),
  ] : [];

  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-400">Finance & Accounts</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Supplier payables</h1><p className="mt-2 max-w-3xl text-sm text-neutral-500">Bills, due dates, partial payments and supplier payment history in one place.</p></div><button onClick={() => { if(mode && !confirmDiscardChanges(billDirty || paymentDirty)) return; setBillForm(blankBill()); setMode("bill"); setSelected(null); setConfirmMode(null); }} className="inline-flex items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 py-2.5 text-sm font-medium text-white"><Plus className="size-4" />Record vendor bill</button></div>
    <AccountingNav />
