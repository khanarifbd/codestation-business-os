...
const DOCUMENT_TYPE_OPTIONS: SearchOption[] = [
  ...
  { value: "vat_certificate", label: "VAT / GST Certificate" },
  { value: "bank_document", label: "Bank Document" },
  { value: "insurance", label: "Insurance Document" },
  { value: "duns_document", label: "D‑U‑N‑S Document" },
...
const inputClass =
  "mt-2 h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 text-sm outline-none transition focus:border-neutral-500";
const textareaClass =
  "mt-2 min-h-28 w-full rounded-xl border border-neutral-200 bg-white px-3 py-3 text-sm outline-none transition focus:border-neutral-500";
...
export default function CompanySettingsPage() {
  ...
  async function uploadDocument(form: HTMLFormElement) {
    const response = await fetch("/api/company-settings/documents/upload", {
      method: "POST",
  ...
  if (!bundle) {
    return (
      <main className="flex min-h-[70vh] items-center justify-center bg-neutral-100 px-6">
    ...
    <main className="min-h-screen bg-neutral-100 text-neutral-950">
      <div className="mx-auto max-w-[1500px] px-5 py-6 sm:px-7 lg:px-8">
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[230px_minmax(0,1fr)]">
          <aside className="h-fit rounded-2xl border bg-white p-3 shadow-sm xl:sticky xl:top-5">
            ...
            <div className="p-6" key={`${activeTab}-${version}`}>
                ...
                >
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <SearchableSelect
                      label="Company size"
                      name="company_size"
                      defaultValue={profile.company_size ?? company.team_size}
              ...
              {activeTab === "legal" ? (
                <div className="space-y-7">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-neutral-500">Search international identifier types or add a custom type.</p>
                  </div>
                  <form
                    onSubmit={(event) => {
                      event.preventDefault();
                  ...
                  >
                    <div className="grid gap-4 md:grid-cols-3">
                      <SearchableSelect label="Type" name="identifier_type" options={IDENTIFIER_TYPE_OPTIONS} placeholder="Select identifier type" allowCustom required />
                      <Field label="Display label" name="label" placeholder="Tax Identification Number" required />
                      <Field label="Value" name="value" required />
                      <SearchableSelect label="Country" name="country_code" options={COUNTRY_OPTIONS} defaultValue={company.country_code} placeholder="Select country" />
                  ...
                  <div className="divide-y rounded-2xl border">
                    {bundle.identifiers.map((item) => (
                      <div key={item.id} className="flex items-center justify-between gap-4 p-4">
                        ...
                        <button type="button" onClick={() => void runSave(() => api(`/identifiers/${item.id}`, "DELETE").then(() => undefined), "Identifier removed")} className="flex size-9 items-center justify-center rounded-lg border text-neutral-400 hover:text-red-600"><Trash2 className="size-4" /></button>
                      </div>
                    ))}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-4 grid gap-4 sm:grid-cols-2">
                          ...
                          <SearchableSelect label="Country" name="country_code" defaultValue={address?.country_code ?? company.country_code} options={COUNTRY_OPTIONS} placeholder="Select country" />
              ...
              {activeTab === "localization" ? (
                ...
                >
                  <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    <SearchableSelect label="Country" name="country_code" defaultValue={company.country_code} options={COUNTRY_OPTIONS} placeholder="Select country" required />
                    <SearchableSelect label="Timezone" name="timezone" defaultValue={company.timezone} options={TIMEZONE_OPTIONS} placeholder="Select timezone" required />
                    <SearchableSelect label="Base currency" name="currency" defaultValue={company.currency} options={CURRENCY_OPTIONS} placeholder="Select currency" required />
                    <SearchableSelect label="Default language" name="default_language" defaultValue={bundle.localization.default_language} options={LANGUAGE_OPTIONS} placeholder="Select language" allowCustom required />
                    <SelectField label="Date format" name="date_format" defaultValue={bundle.localization.date_format} options={["YYYY-MM-DD", "DD/MM/YYYY", "MM/DD/YYYY", "DD-MM-YYYY"]} />
              ...
              {activeTab === "finance" ? (
                ...
                >
                  <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    <SearchableSelect label="Accounting currency" name="accounting_currency" defaultValue={bundle.financial.accounting_currency} options={CURRENCY_OPTIONS} placeholder="Select currency" required />
              ...
              {activeTab === "numbering" ? (
                <div className="space-y-4">
                  <p className="text-sm text-neutral-500">Numbering is isolated by tenant for invoices, quotations, orders, projects, clients and employees.</p>
                ...
                <div className="space-y-6">
                  <form
                    onSubmit={(event) => {
                      ...
                      const formElement = event.currentTarget;
                      void runSave(async () => {
                        await uploadDocument(formElement);
                        formElement.reset();
                      }, "Document uploaded");
                    }}
                    className="rounded-2xl border bg-neutral-50 p-5"
                  >
                    <div className="grid gap-4 md:grid-cols-3">
                      <SearchableSelect label="Document type" name="document_type" options={DOCUMENT_TYPE_OPTIONS} placeholder="Select document type" allowCustom required />
                      ...
                      <label className="block text-sm font-medium md:col-span-2">
                        Document file
                        <div className="mt-2 flex min-h-24 items-center gap-3 rounded-xl border border-dashed border-neutral-300 bg-white px-4 py-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <input
                              ...
                              className="block w-full text-sm file:mr-3 file:rounded-lg file:border-0 file:bg-neutral-950 file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white"
                            />
                            <p className="mt-1 text-xs text-neutral-400">PDF, image, Word or Excel · maximum 20 MB · stored privately on the server.</p>
                          </div>
                        </div>
                      </label>
                      <Field label="Notes" name="notes" />
                    </div>
                    <button disabled={saving} className="mt-5 flex items-center gap-2 rounded-xl bg-neutral-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">
                  ...
                  <div className="divide-y rounded-2xl border">
                    {bundle.documents.map((document) => (
                      <div key={document.id} className="flex flex-col justify-between gap-4 p-4 sm:flex-row sm:items-center">
                        <div>
                        ...
                        <div className="flex items-center gap-2">
                          {document.file_url ? (
                            <a href={document.file_url} target="_blank" rel="noreferrer" className="flex h-9 items-center gap-2 rounded-lg border px-3 text-xs font-semibold hover:bg-neutral-50">
                              <Download className="size-4" /> Download
                            </a>
                          ) : null}
                          <button type="button" onClick={() => void runSave(() => api(`/document-files/${document.id}`, "DELETE").then(() => undefined), "Document removed")} className="flex size-9 items-center justify-center rounded-lg border text-neutral-400 hover:text-red-600"><Trash2 className="size-4" /></button>
                        </div>
                      </div>
                    ))}
...
function Field({
  ...
  return (
    <label className="block text-sm font-medium">
      {label}
      <input name={name} type={type} defaultValue={defaultValue ?? ""} placeholder={placeholder} required={required} step={step} className={inputClass} />
    </label>
  );
}
...
function SaveBar({ saving }: { saving: boolean }) {
  return (
    <div className="flex justify-end border-t pt-5">
      <button type="submit" disabled={saving} className="flex h-11 items-center gap-2 rounded-xl bg-neutral-950 px-5 text-sm font-semibold text-white hover:bg-neutral-800 disabled:opacity-60">
