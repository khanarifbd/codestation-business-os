...
import { CheckCircle2, ClipboardCheck, Loader2, PlayCircle, Search, X, XCircle } from "lucide-react";
...
export default function OrdersPage() {
  ...
  return (
    <main className="min-h-screen bg-neutral-100 p-5 sm:p-8 lg:p-10">
      ...
      {(detailLoading || detail) ? <OrderDrawer detail={detail} loading={detailLoading} saving={saving} onClose={() => { setDetail(null); if (requestedOrderId) router.replace("/dashboard/orders"); }} onStatus={changeStatus} /> : null}
    </main>
  );
}

function OrderDrawer({ detail, loading, saving, onClose, onStatus }: { detail: OrderDetail | null; loading: boolean; saving: boolean; onClose: () => void; onStatus: (status: "in_progress" | "completed" | "cancelled") => Promise<void> }) {
