...
def invoice_page(
    ...
    balance: Literal["all", "outstanding", "paid"] = "all",
    ...
    if balance == "outstanding":
        query = query.where(Invoice.balance_due > 0, Invoice.status.not_in(["draft", "cancelled", "paid"]))
    elif balance == "paid":
        query = query.where(Invoice.balance_due <= 0, Invoice.status == "paid")
    if search:
        needle = f"%{search.strip()}%"
        query = query.where(or_(Invoice.invoice_number.ilike(needle), Invoice.subject.ilike(needle), Invoice.client_name_snapshot.ilike(needle)))
...
def transfer_page(
    ...
    if account_id:
        query = query.where((AccountTransfer.from_account_id == account_id) | (AccountTransfer.to_account_id == account_id))
    if currency:
        code = currency.upper()
        query = query.where(or_(AccountTransfer.source_currency == code, AccountTransfer.destination_currency == code))
    if search:
        needle = f"%{search.strip()}%"
        query = query.where(or_(AccountTransfer.transfer_number.ilike(needle), AccountTransfer.reference.ilike(needle), source_account.name.ilike(needle), destination_account.name.ilike(needle)))
    if cursor:
        date_raw, created_raw, row_id = _decode(cursor, 3)
        transfer_date = date.fromisoformat(date_raw)
        created_at = datetime.fromisoformat(created_raw)
...
def ledger_page(
    ...
    if direction:
        query = query.where(FinancialTransaction.direction == direction)
    if source_type:
        query = query.where(FinancialTransaction.source_type == source_type)
    if date_from:
    ...
    if search:
        needle = f"%{search.strip()}%"
        query = query.where(or_(FinancialTransaction.description.ilike(needle), FinancialTransaction.reference.ilike(needle), FinancialTransaction.source_id.ilike(needle), FinancialTransaction.source_type.ilike(needle)))
    if cursor:
        date_raw, created_raw, row_id = _decode(cursor, 3)
