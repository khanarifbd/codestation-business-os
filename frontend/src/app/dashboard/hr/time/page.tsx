...
type Attendance = { id: string; employee_id: string; employee_name: string | null; attendance_date: string; status: string; attendance_mode?: string | null; work_minutes: number; overtime_minutes: number; notes?: string | null };
...
function AttendanceTable({ rows }: { rows: Attendance[] }) { if (!rows.length) return <Empty text="No attendance records found." />; return <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-sm"><thead className="bg-neutral-50 text-left text-xs uppercase text-neutral-400"><tr><th className="px-5 py-3">Employee</th><th>Date</th><th>Status</th><th>Mode</th><th>Worked</th><th>Overtime</th><th className="pr-5">Note</th></tr></thead><tbody className="divide-y">{rows.map(row => <tr key={row.id}><td className="px-5 py-4 font-medium">{row.employee_name ?? "Employee"}</td><td>{row.attendance_date}</td><td><Status value={row.status} /></td><td className="capitalize text-neutral-500">{row.attendance_mode || "Legacy / manual"}</td><td>{minutes(row.work_minutes)}</td><td>{minutes(row.overtime_minutes)}</td><td className="pr-5 text-neutral-500">{row.notes || "—"}</td></tr>)}</tbody></table></div>; }
