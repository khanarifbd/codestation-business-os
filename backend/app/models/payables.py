...
class PayableBill(TenantOwnedMixin, Base):
    ...
    tax_rate_snapshot: Mapped[Decimal | None] = mapped_column(Numeric(8, 4), nullable=True)
    input_tax_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=Decimal("0"), nullable=False)
    recoverable_tax_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=Decimal("0"), nullable=False)
    withholding_tax_code_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("tax_codes.id", ondelete="SET NULL"), nullable=True)
    withholding_rate_snapshot: Mapped[Decimal | None] = mapped_column(Numeric(8, 4), nullable=True)
    withholding_tax_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), default=Decimal("0"), nullable=False)
    original_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), nullable=False)
    net_payable_amount: Mapped[Decimal] = mapped_column(Numeric(18, 2), nullable=False)
