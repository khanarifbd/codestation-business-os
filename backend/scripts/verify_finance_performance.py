...
from app.api.v1.payables_workspace import payables_workspace
...
def main() -> None:
    ...
    try:
        ...
        payable_result, payable_queries = count_selects(
            lambda: payables_workspace(db, tenant, search=None, status_filter="open", currency=None, limit=50, cursor=None)  # type: ignore[arg-type]
        )
        ...
        if invoice_cursor_queries > 1:
            raise AssertionError(f"invoice cursor query regression: expected <=1 SELECTs, got {invoice_cursor_queries}")
        if receivable_queries > 6:
            raise AssertionError(f"receivable page query regression: expected <=6 SELECTs, got {receivable_queries}")
        if payable_queries > 5:
            raise AssertionError(f"payables workspace query regression: expected <=5 SELECTs, got {payable_queries}")
        ...
        if (
            len(invoice_cursor_result.items) > 50
            or len(payment_cursor_result.items) > 50
            or len(receivable_result.items) > 50
            or len(payable_result.items) > 50
        ):
        ...
        if payable_result.filtered_count < len(payable_result.items):
            raise AssertionError("payables filtered count is smaller than the returned page")
        if sum(1 for item in payable_result.items if item.balance_due > 0) > payable_result.open_bill_count:
            raise AssertionError("payables open summary is inconsistent with the returned page")
    finally:
        db.close()
    ...
    print(
        ...
        f"invoice_cursor={invoice_cursor_queries}, receivables={receivable_queries}, payables={payable_queries}, "
