...
export function ClientNotesSection({ clientId, canManage }: { clientId: string; canManage: boolean }) {
  ...
  >
    ...
    {loading ? <Loading /> : items.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <article key={item.id} className="rounded-2xl border bg-neutral-50 p-3 sm:p-4">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="break-words font-semibold">{item.title}</p><p className="mt-1 text-xs text-neutral-400">Updated {dateTime(item.updated_at)}</p></div>{canManage ? <div className="flex shrink-0 gap-1"><IconButton title="Edit note" onClick={() => openEdit(item)}><Pencil className="size-4" /></IconButton><IconButton title="Delete note" danger onClick={() => void remove(item)}><Trash2 className="size-4" /></IconButton></div> : null}</div>
...
export function ClientDocumentsSection({ clientId, canManage }: { clientId: string; canManage: boolean }) {
  ...
  >
    <Notices error={error} message={message} />
    {loading ? <Loading /> : items.length ? <><div className="grid gap-3 md:hidden">{items.map((item) => <article key={item.id} className="rounded-2xl border bg-neutral-50 p-4">
      <div className="min-w-0"><p className="break-words font-semibold">{item.title}</p><p className="mt-1 break-all text-xs text-neutral-400">{item.original_filename}</p>{item.notes ? <p className="mt-2 break-words text-xs leading-5 text-neutral-500">{item.notes}</p> : null}</div>
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs"><div><p className="text-neutral-400">Type</p><p className="mt-1 font-medium text-neutral-700">{pretty(item.document_type)}</p></div><div><p className="text-neutral-400">Size</p><p className="mt-1 font-medium text-neutral-700">{fileSize(item.size_bytes)}</p></div><div className="col-span-2"><p className="text-neutral-400">Uploaded</p><p className="mt-1 font-medium text-neutral-700">{dateTime(item.created_at)}</p></div></div>
      <div className={`mt-4 grid gap-2 ${canManage ? "grid-cols-3" : "grid-cols-2"}`}><button type="button" onClick={() => window.open(`/api/crm/clients/${encodeURIComponent(clientId)}/documents/${encodeURIComponent(item.id)}/preview`, "_blank", "noopener,noreferrer")} className="h-10 rounded-xl border bg-white px-2 text-xs font-semibold">View</button><a href={`/api/crm/clients/${encodeURIComponent(clientId)}/documents/${encodeURIComponent(item.id)}/file`} className="inline-flex h-10 items-center justify-center gap-1 rounded-xl border bg-white px-2 text-xs font-semibold"><Download className="size-3.5" />Download</a>{canManage ? <button type="button" disabled={saving} onClick={() => void remove(item)} className="h-10 rounded-xl border border-red-100 bg-white px-2 text-xs font-semibold text-red-600 disabled:opacity-50">Delete</button> : null}</div>
...
export function ClientCredentialsSection({ clientId, canManage }: { clientId: string; canManage: boolean }) {
  ...
  >
    ...
    {loading ? <Loading /> : items.length ? <div className="grid gap-4 lg:grid-cols-2">{items.map((item) => <article key={item.id} className="rounded-2xl border bg-neutral-50 p-4 sm:p-5">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="break-words font-semibold">{item.name}</p><span className="rounded-full border bg-white px-2 py-0.5 text-[11px] text-neutral-500">{pretty(item.access_level)}</span></div><p className="mt-1 text-xs text-neutral-400">{pretty(item.credential_type)} · {pretty(item.environment)}</p></div>{canManage ? <div className="flex shrink-0 gap-1"><IconButton title="Edit credential" onClick={() => openEdit(item)}><Pencil className="size-4" /></IconButton><IconButton title="Delete credential" danger onClick={() => void remove(item)}><Trash2 className="size-4" /></IconButton></div> : null}</div>
      ...
      {item.notes ? <p className="mt-4 whitespace-pre-wrap break-words rounded-xl bg-white p-3 text-sm leading-6 text-neutral-600">{item.notes}</p> : null}
      <p className="mt-4 break-words text-xs text-neutral-400">Last revealed {dateTime(item.last_revealed_at)}{item.last_revealed_by ? ` by ${item.last_revealed_by}` : ""}</p>
...
function Panel({ icon: Icon, title, description, action, children }: { icon: typeof FileText; title: string; description: string; action?: React.ReactNode; children: React.ReactNode }) {
  return <section className="rounded-2xl border bg-white p-4 shadow-sm sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-neutral-100"><Icon className="size-5 text-neutral-600" /></div><div className="min-w-0"><h2 className="break-words font-semibold">{title}</h2><p className="mt-1 max-w-3xl break-words text-sm leading-6 text-neutral-500">{description}</p></div></div>{action}</div><div className="mt-4 sm:mt-5">{children}</div><style jsx global>{`.input{height:44px;width:100%;border:1px solid #e5e5e5;border-radius:12px;padding:0 12px;font-size:14px;background:white}.textarea{width:100%;border:1px solid #e5e5e5;border-radius:12px;padding:10px 12px;font-size:14px;background:white}.btn-primary{height:46px;border-radius:12px;background:#0a0a0a;color:white;font-size:14px;font-weight:600}.btn-primary:disabled{opacity:.5}`}</style></section>;
}

function PrimaryButton({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return <button type="button" onClick={onClick} className="inline-flex h-11 w-full shrink-0 items-center justify-center gap-2 rounded-xl bg-neutral-950 px-4 text-sm font-semibold text-white sm:w-auto">{children}</button>;
}
...
function IconButton({ title, danger = false, onClick, children }: { title: string; danger?: boolean; onClick: () => void; children: React.ReactNode }) {
  return <button type="button" title={title} onClick={onClick} className={`flex size-9 shrink-0 items-center justify-center rounded-lg border bg-white ${danger ? "text-red-600" : "text-neutral-500"}`}>{children}</button>;
}

function CredentialLine({ label, value, action }: { label: string; value: string; action?: React.ReactNode }) {
  return <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs text-neutral-400">{label}</p><p className="mt-1 break-all font-medium text-neutral-700">{value}</p></div>{action ? <div className="shrink-0">{action}</div> : null}</div>;
}
...
function Modal({ title, onClose, children, wide = false }: { title: string; onClose: () => void; children: React.ReactNode; wide?: boolean }) {
  return <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-black/45 sm:items-center sm:p-4"><div className={`h-full max-h-[100dvh] w-full overflow-y-auto bg-white p-4 shadow-2xl sm:h-auto sm:max-h-[94vh] sm:rounded-2xl sm:p-6 ${wide ? "sm:max-w-3xl" : "sm:max-w-xl"}`}><div className="sticky top-0 z-10 -mx-4 -mt-4 flex items-center justify-between gap-3 border-b bg-white px-4 py-4 sm:static sm:mx-0 sm:mt-0 sm:border-0 sm:p-0"><h3 className="min-w-0 break-words text-lg font-semibold">{title}</h3><button type="button" onClick={onClose} className="shrink-0 rounded-lg p-2 hover:bg-neutral-100"><X className="size-5" /></button></div><div className="mt-5">{children}</div></div></div>;
}
...
function Empty({ text }: { text: string }) {
  return <div className="py-10 text-center text-sm text-neutral-400 sm:py-12">{text}</div>;
}
