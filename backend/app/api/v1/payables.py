...
def _tax_code(db: DbSession, organization_id: str, tax_code_id: str | None, kind: str, bill_date) -> TaxCode | None:
    ...
    if row is None:
        raise HTTPException(status_code=404, detail=f"Active {kind} tax code not found")
    if row.effective_from and bill_date < row.effective_from:
        raise HTTPException(status_code=400, detail=f"Tax code {row.code} is not effective yet")
    if row.effective_to and bill_date > row.effective_to:
        raise HTTPException(status_code=400, detail=f"Tax code {row.code} has expired")
    return row
...
def _bill_read(db: DbSession, organization_id: str, bill: PayableBill) -> PayableBillRead:
    ...
    return PayableBillRead(
        id=bill.id, bill_number=bill.bill_number, supplier_name=bill.supplier_name, bill_date=bill.bill_date, due_date=bill.due_date, currency=bill.currency,
        subtotal_amount=bill.subtotal_amount, tax_code_id=bill.tax_code_id, tax_rate_snapshot=bill.tax_rate_snapshot, input_tax_amount=bill.input_tax_amount,
        recoverable_tax_amount=bill.recoverable_tax_amount, withholding_tax_code_id=bill.withholding_tax_code_id, withholding_rate_snapshot=bill.withholding_rate_snapshot,
        withholding_tax_amount=bill.withholding_tax_amount, original_amount=bill.original_amount, net_payable_amount=bill.net_payable_amount,
        amount_paid=bill.amount_paid, balance_due=bill.balance_due, expense_ledger_account_id=bill.expense_ledger_account_id,
        expense_ledger_account_name=category_name, description=bill.description, reference=bill.reference, notes=bill.notes, status=bill.status, created_at=bill.created_at,
    )
...
def create_payable_bill(payload: PayableBillCreate, request: Request, db: DbSession, tenant: AccountingManager):
    ...
    subtotal = _money(payload.amount)
    purchase_tax = _tax_code(db, tenant.organization_id, payload.tax_code_id, "purchase", payload.bill_date)
    withholding_tax = _tax_code(db, tenant.organization_id, payload.withholding_tax_code_id, "withholding", payload.bill_date)
    input_tax = _money(subtotal * Decimal(purchase_tax.rate) / Decimal("100")) if purchase_tax else Decimal("0.00")
    recoverable = _money(input_tax * Decimal(purchase_tax.recoverable_percent) / Decimal("100")) if purchase_tax else Decimal("0.00")
    nonrecoverable = _money(input_tax - recoverable)
    gross = _money(subtotal + input_tax)
    withholding = _money(subtotal * Decimal(withholding_tax.rate) / Decimal("100")) if withholding_tax else Decimal("0.00")
    if withholding > gross:
        raise HTTPException(status_code=400, detail="Withholding tax cannot exceed gross vendor bill")
    net_payable = _money(gross - withholding)

    bill = PayableBill(
        organization_id=tenant.organization_id, bill_number=f"BILL-{payload.bill_date.strftime('%Y%m%d')}-{uuid4().hex[:6].upper()}", supplier_name=payload.supplier_name.strip(), bill_date=payload.bill_date,
        due_date=payload.due_date, currency=payload.currency.upper(), subtotal_amount=subtotal, tax_code_id=purchase_tax.id if purchase_tax else None,
        tax_rate_snapshot=purchase_tax.rate if purchase_tax else None, input_tax_amount=input_tax, recoverable_tax_amount=recoverable,
        withholding_tax_code_id=withholding_tax.id if withholding_tax else None, withholding_rate_snapshot=withholding_tax.rate if withholding_tax else None,
        withholding_tax_amount=withholding, original_amount=gross, net_payable_amount=net_payable, amount_paid=Decimal("0"), balance_due=net_payable,
        expense_ledger_account_id=expense.id, description=payload.description.strip(), reference=_clean(payload.reference), notes=_clean(payload.notes), status="open", created_by_user_id=tenant.user_id,
    )
    ...
    if recoverable > 0:
        input_tax_ledger = _tax_ledger(db, tenant.organization_id, tenant.user_id, system_key="input_tax_receivable", code="1210", name="Input Tax Receivable", category="asset", normal_balance="debit")
        lines.append(PostingLine(ledger_account_id=input_tax_ledger.id, debit=recoverable, currency=bill.currency, description=f"Recoverable input tax: {bill.bill_number}"))
    ...
    if withholding > 0:
        withholding_ledger = _tax_ledger(db, tenant.organization_id, tenant.user_id, system_key="withholding_tax_payable", code="2210", name="Withholding Tax Payable", category="liability", normal_balance="credit")
        lines.append(PostingLine(ledger_account_id=withholding_ledger.id, credit=withholding, currency=bill.currency, description=f"Withholding tax: {bill.bill_number}"))
    ...
    record_activity(db, action="accounting.payable.bill_created", scope="tenant", actor_user_id=tenant.user_id, organization_id=tenant.organization_id, entity_type="payable_bill", entity_id=bill.id,
        after={"supplier_name": bill.supplier_name, "subtotal": str(subtotal), "input_tax": str(input_tax), "recoverable_tax": str(recoverable), "withholding_tax": str(withholding), "gross": str(gross), "net_payable": str(net_payable), "currency": bill.currency},
        message=f"Vendor bill recorded: {bill.bill_number} — {bill.currency} {gross}", request=request)
...
def pay_payable_bill(bill_id: str, payload: PayablePaymentCreate, request: Request, db: DbSession, tenant: AccountingManager):
    ...
    payable = system_account(db, tenant.organization_id, "accounts_payable")
    post_journal(db, organization_id=tenant.organization_id, user_id=tenant.user_id, entry_date=payment.payment_date, source_type="payable_payment", source_id=payment.id,
        lines=[PostingLine(ledger_account_id=payable.id, debit=amount, currency=bill.currency, description=f"Payment to {bill.supplier_name}"), PostingLine(ledger_account_id=financial_ledger.id, credit=amount, currency=bill.currency, description=bill.description)], reference=payment.reference, memo=f"Payment for {bill.bill_number}")
