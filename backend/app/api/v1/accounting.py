...
def update_ledger_account(account_id: str, payload: LedgerAccountUpdate, request: Request, db: DbSession, tenant: AccountingManager):
    ...
    if item.system_key and item.system_key.startswith("financial_account:") and changes.get("allow_manual_posting") is True:
        raise HTTPException(status_code=409, detail="Mapped financial-account ledgers cannot accept manual journals. Use Money In, Money Out, Transfer or another financial-account workflow so the account balance and General Ledger stay synchronized.")
    ...
    db.flush()
    record_activity(db, action="accounting.ledger_account.updated", scope="tenant", actor_user_id=tenant.user_id, organization_id=tenant.organization_id, entity_type="ledger_account", entity_id=item.id, before=before, after={"code": item.code, "name": item.name, "is_active": item.is_active, "parent_id": item.parent_id, "allow_manual_posting": item.allow_manual_posting}, message=f"Ledger account updated: {item.code} {item.name}", request=request)
    db.commit(); db.refresh(item)
    return LedgerAccountRead.model_validate(item)
...
def create_manual_journal(payload: JournalEntryCreate, request: Request, db: DbSession, tenant: AccountingManager):
    ...
    for line in payload.lines:
        ...
        if source_currency == base_currency:
            ...
            base_amount, effective_rate = to_base_amount(db, tenant.organization_id, base_currency, original_amount, source_currency, rate_date=payload.entry_date)

        normalized_lines.append({"source": line, "currency": source_currency, "rate": effective_rate, "debit": base_amount if submitted_debit > 0 else Decimal("0"), "credit": base_amount if submitted_credit > 0 else Decimal("0"), "original_amount": original_amount})
...
def reverse_journal(entry_id: str, request: Request, db: DbSession, tenant: AccountingManager):
    ...
    if existing is not None: raise HTTPException(status_code=409, detail=f"Journal entry was already reversed by {existing.entry_number}")
    _ensure_open_period(db, tenant.organization_id, original.entry_date); assert_current_functional_posting_period(db, tenant.organization_id, original.entry_date)
    ...
    if not source_lines: raise HTTPException(status_code=409, detail="Journal entry has no lines to reverse")
    ...
    for line in source_lines: db.add(JournalLine(organization_id=tenant.organization_id, journal_entry_id=reversal.id, ledger_account_id=line.ledger_account_id, description=f"Reversal: {line.description or original.entry_number}", currency=line.currency, exchange_rate_to_base=line.exchange_rate_to_base, debit=line.credit, credit=line.debit, original_amount=line.original_amount))
    db.flush(); record_activity(db, action="accounting.journal.reversed", scope="tenant", actor_user_id=tenant.user_id, organization_id=tenant.organization_id, entity_type="journal_entry", entity_id=reversal.id, before={"original_entry_id": original.id, "original_entry_number": original.entry_number}, after={"reversal_entry_id": reversal.id, "reversal_entry_number": reversal.entry_number, "functional_currency": reversal.functional_currency}, message=f"Journal {original.entry_number} reversed by {reversal.entry_number}", request=request); db.commit(); db.refresh(reversal)
    return _journal_read(db, tenant.organization_id, reversal)


@router.get("/trial-balance", response_model=TrialBalanceRead)
def trial_balance(db: DbSession, tenant: AccountingViewer, as_of: date | None = Query(default=None)):
    report_date = as_of or organization_local_date(tenant.organization)
    period = functional_currency_period_for_date(db, tenant.organization_id, report_date)
    activity = (select(JournalLine.ledger_account_id.label("ledger_account_id"), func.coalesce(func.sum(JournalLine.debit), 0).label("debit"), func.coalesce(func.sum(JournalLine.credit), 0).label("credit")).join(JournalEntry, JournalEntry.id == JournalLine.journal_entry_id).where(JournalLine.organization_id == tenant.organization_id, JournalEntry.organization_id == tenant.organization_id, JournalEntry.status == "posted", JournalEntry.functional_currency == period.currency, JournalEntry.entry_date >= period.effective_from, JournalEntry.entry_date <= report_date).group_by(JournalLine.ledger_account_id).subquery())
    rows = db.execute(select(LedgerAccount.id, LedgerAccount.code, LedgerAccount.name, LedgerAccount.category, func.coalesce(activity.c.debit, 0).label("debit"), func.coalesce(activity.c.credit, 0).label("credit")).outerjoin(activity, activity.c.ledger_account_id == LedgerAccount.id).where(LedgerAccount.organization_id == tenant.organization_id).order_by(LedgerAccount.code.asc())).all()
    result=[]; total_debit=Decimal("0"); total_credit=Decimal("0")
    for account_id, code, name, category, debit, credit in rows:
        debit_value=_money(Decimal(debit or 0)); credit_value=_money(Decimal(credit or 0)); total_debit+=debit_value; total_credit+=credit_value; result.append(TrialBalanceRow(ledger_account_id=account_id, code=code, name=name, category=category, debit=debit_value, credit=credit_value, balance=_money(debit_value-credit_value)))
    return TrialBalanceRead(as_of=report_date, accounting_currency=period.currency.upper(), functional_period_start=period.effective_from, total_debit=_money(total_debit), total_credit=_money(total_credit), rows=result)
