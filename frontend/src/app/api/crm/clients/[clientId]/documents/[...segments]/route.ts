...
type RouteContext = { params: Promise<{ clientId: string; segments: string[] }> };

async function forward(request: NextRequest, context: RouteContext) {
  const { clientId, segments } = await context.params;
  const path = `/crm/clients/${encodeURIComponent(clientId)}/documents/${segments.map(encodeURIComponent).join("/")}${request.nextUrl.search}`;
