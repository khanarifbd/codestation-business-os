...
def _invoice_product_revenue(db, organization_id: str, invoice: Invoice) -> Decimal:
    rows = db.execute(
        select(InvoiceItem.taxable_amount, OrderItem.item_type_snapshot)
        .outerjoin(OrderItem, OrderItem.id == InvoiceItem.source_order_item_id)
        .where(InvoiceItem.organization_id == organization_id, InvoiceItem.invoice_id == invoice.id)
    ).all()
    return money(sum((Decimal(amount or 0) for amount, item_type in rows if item_type in {"stock_item", "non_stock_item"}), Decimal("0")))
...
def sync_operational_accounting(db, *, organization_id: str, user_id: str, base_currency: str) -> dict:
    ...
    for account in accounts:
        ...
        try:
            ...
            post_journal(db,organization_id=organization_id,user_id=user_id,entry_date=account.created_at.date(),source_type="financial_account_opening_sync",source_id=account.id,lines=lines,reference=account.account_reference,memo=f"Opening balance for {account.name}"); counts["opening_balances"] += 1
    ...
    for invoice in invoices:
        if _already_posted(db, organization_id, "invoice_issue", invoice.id): continue
        try:
            ar=system_account(db,organization_id,"accounts_receivable"); service_revenue=system_account(db,organization_id,"service_revenue"); sales_revenue=system_account(db,organization_id,"sales_revenue"); tax_payable=system_account(db,organization_id,"taxes_payable")
            ...
            revenue_base=money(total_base-tax_base); total_revenue_original=money(Decimal(invoice.total)-Decimal(invoice.tax_total))
            product_original=min(_invoice_product_revenue(db,organization_id,invoice),total_revenue_original)
            product_base=money(product_original*rate); service_base=money(revenue_base-product_base); service_original=money(total_revenue_original-product_original)
            ...
            if product_base>0: lines.append(PostingLine(ledger_account_id=sales_revenue.id,credit=product_base,currency=invoice.currency,exchange_rate_to_base=rate,original_amount=product_original,description=f"Product sales · {invoice.subject or invoice.invoice_number}"))
            if service_base>0: lines.append(PostingLine(ledger_account_id=service_revenue.id,credit=service_base,currency=invoice.currency,exchange_rate_to_base=rate,original_amount=service_original,description=f"Service revenue · {invoice.subject or invoice.invoice_number}"))
    ...
    for payment in payments:
        if _already_posted(db,organization_id,"invoice_payment",payment.id): continue
        try:
            ...
            base_amount,cash_rate=to_base_amount(db,organization_id,base_currency,Decimal(payment.account_amount),payment.account_currency);settlement_rate=(base_amount/Decimal(payment.invoice_amount)) if Decimal(payment.invoice_amount) else Decimal("1")
            lines=[PostingLine(ledger_account_id=cash_ledger.id,debit=base_amount,currency=payment.account_currency,exchange_rate_to_base=cash_rate,original_amount=payment.account_amount,description=f"Payment {payment.payment_number}"),PostingLine(ledger_account_id=ar.id,credit=base_amount,currency=payment.invoice_currency,exchange_rate_to_base=settlement_rate,original_amount=payment.invoice_amount,description=f"Payment {payment.payment_number}")]
    ...
    for expense,cost_type in expense_rows:
        if _already_posted(db,organization_id,"expense_post",expense.id): continue
        try:
            ...
            base_amount,cash_rate=to_base_amount(db,organization_id,base_currency,Decimal(expense.account_amount),expense.account_currency);source_rate=(base_amount/Decimal(expense.expense_amount)) if Decimal(expense.expense_amount) else Decimal("1")
    ...
    for transfer in transfers:
        if _already_posted(db,organization_id,"account_transfer",transfer.id): continue
        try:
            ...
            post_journal(db,organization_id=organization_id,user_id=user_id,entry_date=transfer.transfer_date,source_type="account_transfer",source_id=transfer.id,lines=lines,reference=transfer.reference or transfer.transfer_number,memo=f"Account transfer {transfer.transfer_number}");counts["transfers"]+=1
    ...
    for run,period in paid_payroll:
        if _already_posted(db,organization_id,"payroll_payment",run.id): continue
        try:
            ...
            gross_base,rate=to_base_amount(db,organization_id,base_currency,Decimal(run.gross_total),run.currency);net_base,_=to_base_amount(db,organization_id,base_currency,Decimal(run.net_total),run.currency);withheld_original=money(Decimal(run.gross_total)-Decimal(run.net_total));withheld_base=money(gross_base-net_base)
    ...
    for investment in investments:
        if _already_posted(db,organization_id,"company_investment",investment.id): continue
        try:
            _,cash_ledger=financial_ledger_account(db,organization_id,investment.account_id);investment_ledger=system_account(db,organization_id,"investments");base_amount,rate=to_base_amount(db,organization_id,base_currency,Decimal(investment.invested_amount),investment.currency)
    ...
    for item,investment in investment_returns:
        if _already_posted(db,organization_id,"investment_return",item.id): continue
        try:
            _,cash_ledger=financial_ledger_account(db,organization_id,item.account_id);investment_ledger=system_account(db,organization_id,"investments");income_ledger=system_account(db,organization_id,"other_income");cash_base,rate=to_base_amount(db,organization_id,base_currency,Decimal(item.cash_amount),investment.currency);principal_base,_=to_base_amount(db,organization_id,base_currency,Decimal(item.principal_return_amount),investment.currency) if Decimal(item.principal_return_amount)>0 else (Decimal("0"),rate);income_base=money(cash_base-principal_base);lines=[PostingLine(ledger_account_id=cash_ledger.id,debit=cash_base,currency=investment.currency,exchange_rate_to_base=rate,original_amount=item.cash_amount,description=investment.investee_name)]
    ...
    for investor in investor_funding:
        if _already_posted(db,organization_id,"project_investor_funding",investor.id): continue
        try:
            _,cash_ledger=financial_ledger_account(db,organization_id,investor.account_id);funding_ledger=system_account(db,organization_id,"investor_funds_payable");base_amount,rate=to_base_amount(db,organization_id,base_currency,Decimal(investor.invested_amount),investor.currency)
    ...
    for payout,investor in payouts:
        if _already_posted(db,organization_id,"investor_payout",payout.id): continue
        try:
            _,cash_ledger=financial_ledger_account(db,organization_id,payout.account_id);funding_ledger=system_account(db,organization_id,"investor_funds_payable");profit_expense=system_account(db,organization_id,"investor_profit_share");total_original=money(Decimal(payout.principal_return_amount)+Decimal(payout.profit_share_amount));total_base,rate=to_base_amount(db,organization_id,base_currency,total_original,investor.currency);principal_base,_=to_base_amount(db,organization_id,base_currency,Decimal(payout.principal_return_amount),investor.currency) if Decimal(payout.principal_return_amount)>0 else (Decimal("0"),rate);profit_base=money(total_base-principal_base);lines=[PostingLine(ledger_account_id=cash_ledger.id,credit=total_base,currency=investor.currency,exchange_rate_to_base=rate,original_amount=total_original,description=investor.investor_name)]
