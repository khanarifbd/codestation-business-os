...
export default function OrdersPage() {
  ...
  async function changeStatus(next: "in_progress" | "completed" | "cancelled") {
    if (!detail) return;
    if (next === "completed" && !window.confirm("Complete this order? This marks the order as finished and may lock downstream actions until it is reopened.")) return;
    if (detail.status === "completed" && next === "in_progress" && !window.confirm("Reopen this order? The order will return to In Progress and project creation will be available again.")) return;
    setSaving(true);
    setError(null);
    try {
      ...
      setDetail(updated);
      setMessage(`Order ${updated.order_number} ${detail.status === "completed" && updated.status === "in_progress" ? "reopened" : `marked ${updated.status.replace("_", " ")}`}`);
      setRows((current) => {
