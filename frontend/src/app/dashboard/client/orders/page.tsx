...
export default function ClientOrdersPage() {
  ...
  return <main className="min-h-screen bg-neutral-100 p-5 sm:p-8 lg:p-10"><div className="mx-auto max-w-[1200px]">
    ...
    {orders.length ? <div className="mt-6 grid gap-4 lg:grid-cols-2">{orders.map((order) => <Link key={order.id} href={`/dashboard/client/orders/${order.id}`} className="group rounded-2xl border bg-white p-5 transition hover:border-neutral-300 hover:shadow-sm"><div className="flex items-start justify-between gap-4"><div className="flex min-w-0 gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-neutral-100"><ReceiptText className="size-5 text-neutral-500" /></div><div className="min-w-0"><p className="truncate font-semibold">{order.subject || order.order_number}</p><p className="mt-1 text-xs text-neutral-400">{order.order_number} · {formatPortalDate(order.order_date)}</p></div></div><ClientPortalStatusBadge status={order.status} /></div><div className="mt-5 flex items-end justify-between gap-4 border-t pt-4"><div><p className="text-xs text-neutral-400">Order value</p><p className="mt-1 font-semibold">{formatPortalMoney(order.total, order.currency)}</p></div><ArrowRight className="size-4 text-neutral-300 transition group-hover:translate-x-1 group-hover:text-neutral-600" /></div></Link>)}</div> : <div className="mt-6 rounded-2xl border bg-white px-5 py-16 text-center"><ReceiptText className="mx-auto size-8 text-neutral-300" /><p className="mt-3 text-sm font-medium">No orders yet</p><p className="mt-1 text-sm text-neutral-400">Orders linked to your client profile will appear here.</p></div>}
  </div></main>;
}
