...
type InvoicePage = { items: Invoice[]; next_cursor: string | null };
...
export function InvoicesWorkspace() {
  ...
  const loadMeta = useCallback(async () => {
    const response = await fetch("/api/finance/meta", { cache: "no-store" });
    const payload = await response.json();
    if (!response.ok) throw new Error(getApiErrorMessage(payload, "Could not load invoice setup"));
    setMeta(payload);
  }, []);
  ...
  const loadInvoices = useCallback(async (cursor: string | null) => {
    ...
    try {
      const params = new URLSearchParams({ limit: String(pageSize), balance: balanceFilter });
      if (cursor) params.set("cursor", cursor);
      if (search.trim()) params.set("search", search.trim());
      if (statusFilter !== "all") params.set("status", statusFilter);
      if (currencyFilter !== "all") params.set("currency", currencyFilter);
      const response = await fetch(`/api/finance/invoice-page?${params.toString()}`, { cache: "no-store" });
      const payload = await response.json();
      if (!response.ok) throw new Error(getApiErrorMessage(payload, "Could not load invoices"));
      const typed = payload as InvoicePage;
      setInvoices(typed.items ?? []);
  ...
  }, [pageSize, balanceFilter, search, statusFilter, currencyFilter]);

  useEffect(() => { void loadMeta().catch((reason) => setError(reason instanceof Error ? reason.message : "Could not load invoice setup")); }, [loadMeta]);
  useEffect(() => {
    const timer = window.setTimeout(() => void loadInvoices(currentCursor), 250);
    return () => window.clearTimeout(timer);
  }, [loadInvoices, currentCursor]);
  ...
  function setFilter(action: () => void) { action(); resetPaging(); }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setShowForm(false); resetPaging(); await loadInvoices(null);
  ...
  async function sendInvoice(invoice: Invoice) {
    ...
    try {
      ...
      setMessage(`Invoice ${invoice.invoice_number} is now open for collection.`); await loadInvoices(currentCursor);
  ...
  const clientOptions = useMemo(() => meta.clients.map((c) => ({ value: c.id, label: `${c.code} · ${c.name}`, keywords: `${c.code} ${c.name} ${c.currency ?? ""}` })), [meta.clients]);
  const activeFilters = Boolean(search || statusFilter !== "all" || currencyFilter !== "all" || balanceFilter !== "all");
  ...
  return <main className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {showForm ? <section className="rounded-2xl border bg-white p-5">
      ...
      <form onSubmit={submit} className="mt-5 space-y-4">
        ...
        {source === "client" ? <><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <SearchableSelect label="Currency" required value={form.currency} onValueChange={(value) => setForm((v) => ({ ...v, currency: value }))} options={CURRENCY_OPTIONS} placeholder="Select currency" searchPlaceholder="Search currency..." clearable={false} />
        ...
        </div><div><div className="mb-2 flex items-center justify-between"><div><h3 className="font-medium">Invoice items</h3><p className="mt-1 text-xs text-neutral-400">Add what you are billing the client for.</p></div><button type="button" onClick={() => setForm((v) => ({ ...v, lines: [...v.lines, blank()] }))} className="text-sm font-medium">+ Add item</button></div>
          ...
          <div className="space-y-3">{form.lines.map((line, index) => <div key={index} className="grid gap-3 rounded-xl border p-3 lg:grid-cols-6">
            <LabeledInput label="Description" className="lg:col-span-2"><input required value={line.description} onChange={(e) => setForm((v) => ({ ...v, lines: v.lines.map((x, i) => i === index ? { ...x, description: e.target.value } : x) }))} className="w-full rounded-lg border px-3 py-2" /></LabeledInput>
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      <div className="border-b p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="font-semibold">Invoice list</h2><p className="mt-1 text-sm text-neutral-500">Server-side search and pagination keeps large invoice histories fast.</p></div><span className="text-xs text-neutral-400">{invoices.length} shown on page {page}</span></div>
        <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-[1fr_180px_160px_180px_auto]"><input value={search} onChange={(e) => setFilter(() => setSearch(e.target.value))} placeholder="Search invoice, client or subject…" className="rounded-xl border px-3 py-2.5 text-sm"/><select value={statusFilter} onChange={(e) => setFilter(() => setStatusFilter(e.target.value))} className="rounded-xl border bg-white px-3 py-2.5 text-sm"><option value="all">All statuses</option><option value="draft">Draft</option><option value="sent">Sent</option><option value="partially_paid">Partially paid</option><option value="paid">Paid</option><option value="cancelled">Cancelled</option><option value="overdue">Overdue</option></select><select value={currencyFilter} onChange={(e) => setFilter(() => setCurrencyFilter(e.target.value))} className="rounded-xl border bg-white px-3 py-2.5 text-sm"><option value="all">All currencies</option>{CURRENCY_OPTIONS.map((item) => <option key={item.value} value={item.value}>{item.value}</option>)}</select><select value={balanceFilter} onChange={(e) => setFilter(() => setBalanceFilter(e.target.value))} className="rounded-xl border bg-white px-3 py-2.5 text-sm"><option value="all">All balances</option><option value="outstanding">Outstanding only</option><option value="paid">Paid only</option></select>{activeFilters ? <button onClick={() => { setSearch(""); setStatusFilter("all"); setCurrencyFilter("all"); setBalanceFilter("all"); resetPaging(); }} className="rounded-xl border px-3 py-2.5 text-sm">Clear filters</button> : <span />}</div>
      </div>
      <div className="overflow-x-auto"><table className="min-w-[1100px] w-full text-sm"><thead><tr className="border-b text-left text-xs uppercase tracking-wide text-neutral-400"><th className="px-4 py-3">Invoice</th><th className="px-4 py-3">Client</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Issue / Due</th><th className="px-4 py-3">Total</th><th className="px-4 py-3">Paid</th><th className="px-4 py-3">Due</th><th className="px-4 py-3"></th></tr></thead><tbody>{invoices.map((invoice) => <tr key={invoice.id} className="border-b last:border-0 hover:bg-neutral-50"><td className="px-4 py-3"><Link href={`/dashboard/accounting/invoices/${invoice.id}`} className="font-medium hover:underline">{invoice.invoice_number}</Link><p className="text-xs text-neutral-400">{invoice.subject || "No subject"}</p></td><td className="px-4 py-3">{invoice.client_name}</td><td className="px-4 py-3 capitalize">{invoice.display_status.replaceAll("_", " ")}</td><td className="px-4 py-3"><p>{invoice.issue_date}</p><p className="text-xs text-neutral-400">Due {invoice.due_date || "—"}</p></td><td className="px-4 py-3">{money(invoice.total, invoice.currency)}</td><td className="px-4 py-3">{money(invoice.amount_paid, invoice.currency)}</td><td className="px-4 py-3 font-semibold">{money(invoice.balance_due, invoice.currency)}</td><td className="px-4 py-3"><div className="flex justify-end gap-2"><Link href={`/dashboard/accounting/invoices/${invoice.id}`} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-medium"><Eye className="size-3.5" />{invoice.status === "draft" ? "Open / Edit" : "Open"}</Link>{invoice.status === "draft" ? <button disabled={saving} onClick={() => void sendInvoice(invoice)} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-medium"><Send className="size-3.5" />Send</button> : Number(invoice.balance_due) > 0 ? <Link href={`/dashboard/accounting/money-in?invoice=${encodeURIComponent(invoice.id)}`} className="rounded-lg bg-neutral-950 px-3 py-2 text-xs font-medium text-white">Collect</Link> : null}</div></td></tr>)}</tbody></table>{!loading && !invoices.length ? <div className="py-12 text-center"><FileText className="mx-auto size-8 text-neutral-300" /><p className="mt-3 font-medium">No matching invoices</p><p className="mt-1 text-sm text-neutral-400">Change the filters or create a new invoice.</p></div> : null}</div>
      <CursorPager page={page} pageSize={pageSize} shownCount={invoices.length} hasPrevious={cursorStack.length > 1} hasNext={Boolean(nextCursor)} loading={loading} onPrevious={() => setCursorStack((stack) => stack.length > 1 ? stack.slice(0, -1) : stack)} onNext={() => { if (nextCursor) setCursorStack((stack) => [...stack, nextCursor]); }} onPageSizeChange={(value) => { setPageSize(value); resetPaging(); }} />
    </section>
  </div></main>;
}
