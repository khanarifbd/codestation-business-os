...
class AssetCreate(BaseModel):
    ...
    opening_accumulated_depreciation: Decimal = Field(default=0, ge=0)
    ...
    def validate_asset(self):
        ...
        if self.record_mode == "opening":
            if self.opening_balance_date is None: raise ValueError("Opening balance date is required for an existing asset")
            if self.opening_balance_date < self.in_service_date: raise ValueError("Opening balance date cannot be before the in-service date")
            if self.opening_accumulated_depreciation > depreciable: raise ValueError("Opening accumulated depreciation cannot exceed the depreciable amount")
        elif self.opening_accumulated_depreciation != 0 or self.opening_balance_date is not None:
            raise ValueError("Opening balance fields are only valid for existing/opening assets")
        return self
...
def create_asset(payload:AssetCreate,request:Request,db:DbSession,tenant:Manager):
    ...
    opening_accumulated=money(payload.opening_accumulated_depreciation if payload.record_mode=="opening" else 0)
    maximum=money(cost-salvage)
    row=FixedAsset(organization_id=tenant.organization_id,asset_code=payload.asset_code.strip().upper(),name=payload.name.strip(),category=payload.category.strip().lower().replace(" ","_"),currency=currency,acquisition_cost=cost,salvage_value=salvage,accumulated_depreciation=opening_accumulated,opening_accumulated_depreciation=opening_accumulated,record_mode=payload.record_mode,opening_balance_date=payload.opening_balance_date if payload.record_mode=="opening" else None,acquisition_date=payload.acquisition_date,in_service_date=payload.in_service_date,useful_life_months=payload.useful_life_months,depreciation_method=payload.depreciation_method,purchase_account_id=payload.purchase_account_id if payload.record_mode=="purchase" else None,reference=payload.reference,status="fully_depreciated" if opening_accumulated>=maximum else "active",notes=payload.notes,created_by_user_id=tenant.user_id)
    db.add(row); db.flush(); description=f"Fixed asset {'opening balance' if payload.record_mode=='opening' else 'acquisition'}: {row.asset_code} {row.name}"; fixed=system_account(db,tenant.organization_id,"fixed_assets")
    ...
    posting_date=row.acquisition_date
    lines=[PostingLine(ledger_account_id=fixed.id,debit=base,description=description,currency=currency,exchange_rate_to_base=rate,original_amount=cost)]
    if payload.record_mode=="purchase":
        ...
        lines.append(PostingLine(ledger_account_id=cash_ledger.id,credit=base,description=description,currency=currency,exchange_rate_to_base=rate,original_amount=cost))
    else:
        assert row.opening_balance_date is not None
        posting_date=row.opening_balance_date
        accumulated_ledger=system_account(db,tenant.organization_id,"accumulated_depreciation")
        opening_equity=system_account(db,tenant.organization_id,"opening_balance_equity")
        opening_dep_base=money(opening_accumulated*rate)
        opening_book_base=money(base-opening_dep_base)
        if opening_accumulated>0:
            lines.append(PostingLine(ledger_account_id=accumulated_ledger.id,credit=opening_dep_base,description=f"Opening accumulated depreciation: {row.asset_code}",currency=currency,exchange_rate_to_base=rate,original_amount=opening_accumulated))
        if opening_book_base>0:
            lines.append(PostingLine(ledger_account_id=opening_equity.id,credit=opening_book_base,description=f"Opening net book value: {row.asset_code}",currency=currency,exchange_rate_to_base=rate,original_amount=money(cost-opening_accumulated)))
    post_journal(db,organization_id=tenant.organization_id,user_id=tenant.user_id,entry_date=posting_date,source_type="fixed_asset_acquisition",source_id=row.id,reference=row.reference,memo=description,lines=lines)
