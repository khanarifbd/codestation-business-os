...
type FinancialRow = { currency: string; invoiced_revenue: string; direct_income: string; collected_revenue: string; receivables: string; expenses: string; platform_fees: string; transfer_fees: string; net_profit: string };
type TrendRow = { period: string; currency: string; invoiced_revenue: string; direct_income: string; collected_revenue: string; expenses: string; transfer_fees: string; net_profit: string };
...
export default function ReportsPage() {
  ...
  const maxTrend = useMemo(() => Math.max(1, ...(data?.trend.map((row) => Math.max(Number(row.invoiced_revenue), Number(row.direct_income), Number(row.expenses), Number(row.collected_revenue))) ?? [1])), [data]);
